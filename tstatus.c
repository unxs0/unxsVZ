/*
FILE
	tStatus source code of unxsVZ.cgi
	Built by mysqlRAD2.cgi (C) Gary Wallis 2001-2007
PURPOSE
	Container status deploy state codes.
AUTHOR/LEGAL
	...
	if(!cResult[0])
	{
		...
		if(mysql_errno(&gMysql))
			...
			if(strstr(mysql_error(&gMysql)," doesn't exist"))
                	{
				CreatetStatus();
				unxsVZ("New tStatus table created");
                	}
			else
			{
...
void DeletetStatus(void)
{
	...
	{
#ifdef ISM3FIELDS
		unxsVZLog(uStatus,"tStatus","DelError");
#endif
...
void ModtStatus(void)
{
	...
	sprintf(gcQuery,"SELECT tStatus.uStatus,\
				...
				WHERE tStatus.uStatus=%u\
				AND tStatus.uOwner=tClient.uClient\
				AND (tClient.uOwner=%u OR tClient.uClient=%u)"
			,uStatus,guLoginClient,guLoginClient);
	else
	sprintf(gcQuery,"SELECT uStatus,uModDate FROM tStatus\
				WHERE uStatus=%u"
						,uStatus);
#else
	sprintf(gcQuery,"SELECT uStatus FROM tStatus\
...
void tStatusList(void)
{
	...
	for(guN=0;guN<(guEnd-guStart+1);guN++)
	{
		...
		if(luTime6)
			...
			sprintf(cBuf6,"---");
		printf("<td><a class=darkLink href=unxsVZ.cgi?gcFunction=tStatus&uStatus=%s>%s</a>"
			,field[0]
			,field[0]);
...
{
	sprintf(gcQuery,"CREATE TABLE IF NOT EXISTS tStatus ("
