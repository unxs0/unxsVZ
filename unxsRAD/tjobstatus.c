/*
FILE
	tJobStatus source code of unxsRAD.cgi
	...
	if(!cResult[0])
	{
		...
		if(mysql_errno(&gMysql))
			...
			if(strstr(mysql_error(&gMysql)," doesn't exist"))
                	{
				CreatetJobStatus();
				unxsRAD("New tJobStatus table created");
                	}
			else
			{
...
void NewtJobStatus(unsigned uMode)
{
	...
	unxsRADLog(uJobStatus,"tJobStatus","New");
#endif
...
void DeletetJobStatus(void)
{
	...
	{
#ifdef ISM3FIELDS
		unxsRADLog(uJobStatus,"tJobStatus","Del");
#endif
	...
	{
#ifdef ISM3FIELDS
		unxsRADLog(uJobStatus,"tJobStatus","DelError");
#endif
...
void ModtJobStatus(void)
{
	...
	unxsRADLog(uJobStatus,"tJobStatus","Mod");
#endif
	tJobStatus(gcQuery);
