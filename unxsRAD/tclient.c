	...
	if(!cResult[0])
	{
		...
		if(mysql_errno(&gMysql))
			...
			if(strstr(mysql_error(&gMysql)," doesn't exist"))
                	{
				CreatetClient();
				unxsRAD("New tClient table created");
                	}
			else
			{
...
void NewtClient(unsigned uMode)
{
	...
	unxsRADLog(uClient,"tClient","New");
#endif
...
void DeletetClient(void)
{
	...
	{
#ifdef ISM3FIELDS
		unxsRADLog(uClient,"tClient","Del");
#endif
	...
	{
#ifdef ISM3FIELDS
		unxsRADLog(uClient,"tClient","DelError");
#endif
...
void ModtClient(void)
{
	...
	unxsRADLog(uClient,"tClient","Mod");
#endif
	tClient(gcQuery);
