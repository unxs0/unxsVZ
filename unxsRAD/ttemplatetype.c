...
void ProcesstTemplateTypeListVars(pentry entries[], int x);
 ...
 //In tTemplateTypefunc.c file included below
	...
	if(!cResult[0])
	{
		...
		if(mysql_errno(&gMysql))
			...
			if(strstr(mysql_error(&gMysql)," doesn't exist"))
                	{
				CreatetTemplateType();
				unxsRAD("New tTemplateType table created");
                	}
			else
			{
...
void NewtTemplateType(unsigned uMode)
{
	...
	unxsRADLog(uTemplateType,"tTemplateType","New");
#endif
...
void DeletetTemplateType(void)
{
	...
	{
#ifdef ISM3FIELDS
		unxsRADLog(uTemplateType,"tTemplateType","Del");
#endif
	...
	{
#ifdef ISM3FIELDS
		unxsRADLog(uTemplateType,"tTemplateType","DelError");
#endif
...
void ModtTemplateType(void)
{
	...
	unxsRADLog(uTemplateType,"tTemplateType","Mod");
#endif
	tTemplateType(gcQuery);
