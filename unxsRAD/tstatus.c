/*
FILE
	tStatus source code of unxsRAD.cgi
	...
	if(!cResult[0])
	{
		...
		if(mysql_errno(&gMysql))
			...
			if(strstr(mysql_error(&gMysql)," doesn't exist"))
                	{
				CreatetStatus();
				unxsRAD("New tStatus table created");
                	}
			else
			{
...
void NewtStatus(unsigned uMode)
{
	...
	unxsRADLog(uStatus,"tStatus","New");
#endif
...
void DeletetStatus(void)
{
	...
	{
#ifdef ISM3FIELDS
		unxsRADLog(uStatus,"tStatus","Del");
#endif
	...
	{
#ifdef ISM3FIELDS
		unxsRADLog(uStatus,"tStatus","DelError");
#endif
...
void ModtStatus(void)
{
	...
	unxsRADLog(uStatus,"tStatus","Mod");
#endif
	tStatus(gcQuery);
