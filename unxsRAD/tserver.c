/*
FILE
	tServer source code of unxsRAD.cgi
	...
	if(!cResult[0])
	{
		...
		if(mysql_errno(&gMysql))
			...
			if(strstr(mysql_error(&gMysql)," doesn't exist"))
                	{
				CreatetServer();
				unxsRAD("New tServer table created");
                	}
			else
			{
...
void NewtServer(unsigned uMode)
{
	...
	unxsRADLog(uServer,"tServer","New");
#endif
...
void DeletetServer(void)
{
	...
	{
#ifdef ISM3FIELDS
		unxsRADLog(uServer,"tServer","Del");
#endif
	...
	{
#ifdef ISM3FIELDS
		unxsRADLog(uServer,"tServer","DelError");
#endif
...
void ModtServer(void)
{
	...
	unxsRADLog(uServer,"tServer","Mod");
#endif
	tServer(gcQuery);
