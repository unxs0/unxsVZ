...
//uField
	...
	printf("<input title='%s' type=text name=uField value=%u size=16 maxlength=10 ",LANG_FT_tField_uField,uField);
...
//cLabel
	...
	printf("<input title='%s' type=text name=cLabel value=\"%s\" size=40 maxlength=32 ",LANG_FT_tField_cLabel,EncodeDoubleQuotes(cLabel));
...
//uHtmlXSize
	...
	printf("<input title='%s' type=text name=uHtmlXSize value=%u size=16 maxlength=10 ",LANG_FT_tField_uHtmlXSize,uHtmlXSize);
	if(guPermLevel>=7 && uMode)
	{
		printf("></td></tr>\n");
	}
...
//uHtmlYSize
	...
	printf("<input title='%s' type=text name=uHtmlYSize value=%u size=16 maxlength=10 ",LANG_FT_tField_uHtmlYSize,uHtmlYSize);
	if(guPermLevel>=7 && uMode)
	{
		printf("></td></tr>\n");
	}
...
//uHtmlMax
	...
	printf("<input title='%s' type=text name=uHtmlMax value=%u size=16 maxlength=10 ",LANG_FT_tField_uHtmlMax,uHtmlMax);
	if(guPermLevel>=7 && uMode)
	{
		printf("></td></tr>\n");
	}
...
//uModLevel
	...
	printf("<input title='%s' type=text name=uModLevel value=%u size=16 maxlength=10 ",LANG_FT_tField_uModLevel,uModLevel);
	if(guPermLevel>=7 && uMode)
	{
		printf("></td></tr>\n");
	}
...
void NewtField(unsigned uMode)
{
	...
	sprintf(gcQuery,"SELECT uField FROM tField WHERE uField=%u",uField);
...
void tFieldList(void)
{
	...
	printf("<tr bgcolor=black>"
		"<td><font color=white>uField"
		"<td><font color=white>cLabel"
		"<td><font color=white>uProject"
		"<td><font color=white>uTable"
		"<td><font color=white>uOrder"
		"<td><font color=white>uFieldType"
		"<td><font color=white>uIndexType"
		"<td><font color=white>cFKSpec"
		"<td><font color=white>cExtIndex"
		"<td><font color=white>cTitle"
		"<td><font color=white>uSQLSize"
		"<td><font color=white>uHtmlXSize"
		"<td><font color=white>uHtmlYSize"
	...
	for(guN=0;guN<(guEnd-guStart+1);guN++)
	{
		...
		char cBuf3[128];
		sprintf(cBuf3,"<a class=darkLink href=?gcFunction=tTable&uTable=%.32s>%.32s</a>",
				field[3],
				ForeignKey("tTable","cLabel",strtoul(field[3],NULL,10)));
		char cBuf5[128];
		sprintf(cBuf5,"<a class=darkLink href=?gcFunction=tFieldType&uFieldType=%.32s>%.32s</a>",
				field[5],
				ForeignKey("tFieldType","cLabel",strtoul(field[5],NULL,10)));
		char cBuf6[128];
		sprintf(cBuf6,"<a class=darkLink href=?gcFunction=tIndexType&uIndexType=%.32s>%.32s</a>",
				field[6],
		...
		char cBuf19[128];
		sprintf(cBuf19,"<a class=darkLink href=?gcFunction=tClient&uClient=%.32s>%.32s</a>",
				field[19],
		...
		if(luTime20)
			...
			sprintf(cBuf20,"---");
		char cBuf21[128];
		sprintf(cBuf21,"<a class=darkLink href=?gcFunction=tClient&uClient=%.32s>%.32s</a>",
				field[21],
...
{
	sprintf(gcQuery,"CREATE TABLE IF NOT EXISTS tField ("
				" uField INT UNSIGNED PRIMARY KEY AUTO_INCREMENT,"
				...
				" uTable INT UNSIGNED NOT NULL DEFAULT 0,"
				" uFieldType INT UNSIGNED NOT NULL DEFAULT 0,"
				" uProject INT UNSIGNED NOT NULL DEFAULT 0,"
				" uIndexType INT UNSIGNED NOT NULL DEFAULT 0,"
				" cFKSpec VARCHAR(100) NOT NULL DEFAULT '',"
				...
				" uHtmlMax INT UNSIGNED NOT NULL DEFAULT 0,"
