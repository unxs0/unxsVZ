	...
	if(!cResult[0])
	{
		...
		if(mysql_errno(&gMysql))
			...
			if(strstr(mysql_error(&gMysql)," doesn't exist"))
                	{
				CreatetTemplateType();
				{{cProject}}("New tTemplateType table created");
                	}
			else
			{
...
void NewtTemplateType(unsigned uMode)
{
	...
	{{cProject}}Log(uTemplateType,"tTemplateType","New");
#endif
...
void DeletetTemplateType(void)
{
	...
	{
#ifdef ISM3FIELDS
		{{cProject}}Log(uTemplateType,"tTemplateType","Del");
#endif
	...
	{
#ifdef ISM3FIELDS
		{{cProject}}Log(uTemplateType,"tTemplateType","DelError");
#endif
...
void ModtTemplateType(void)
{
	...
	{{cProject}}Log(uTemplateType,"tTemplateType","Mod");
#endif
	tTemplateType(gcQuery);
