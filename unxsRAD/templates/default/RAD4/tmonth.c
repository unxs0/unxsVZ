/*
FILE
	tMonth source code of {{cProject}}.cgi
	...
	if(!cResult[0])
	{
		...
		if(mysql_errno(&gMysql))
			...
			if(strstr(mysql_error(&gMysql)," doesn't exist"))
                	{
				CreatetMonth();
				{{cProject}}("New tMonth table created");
                	}
			else
			{
...
void NewtMonth(unsigned uMode)
{
	...
	{{cProject}}Log(uMonth,"tMonth","New");
#endif
...
void DeletetMonth(void)
{
	...
	{
#ifdef ISM3FIELDS
		{{cProject}}Log(uMonth,"tMonth","Del");
#endif
	...
	{
#ifdef ISM3FIELDS
		{{cProject}}Log(uMonth,"tMonth","DelError");
#endif
...
void ModtMonth(void)
{
	...
	{{cProject}}Log(uMonth,"tMonth","Mod");
#endif
	tMonth(gcQuery);
