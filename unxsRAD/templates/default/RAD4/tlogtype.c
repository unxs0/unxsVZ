	...
	if(!cResult[0])
	{
		...
		if(mysql_errno(&gMysql))
			...
			if(strstr(mysql_error(&gMysql)," doesn't exist"))
                	{
				CreatetLogType();
				{{cProject}}("New tLogType table created");
                	}
			else
			{
...
void NewtLogType(unsigned uMode)
{
	...
	{{cProject}}Log(uLogType,"tLogType","New");
#endif
...
void DeletetLogType(void)
{
	...
	{
#ifdef ISM3FIELDS
		{{cProject}}Log(uLogType,"tLogType","Del");
#endif
	...
	{
#ifdef ISM3FIELDS
		{{cProject}}Log(uLogType,"tLogType","DelError");
#endif
...
void ModtLogType(void)
{
	...
	{{cProject}}Log(uLogType,"tLogType","Mod");
#endif
	tLogType(gcQuery);
