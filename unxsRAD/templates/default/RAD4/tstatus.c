/*
FILE
	tStatus source code of {{cProject}}.cgi
	...
	if(!cResult[0])
	{
		...
		if(mysql_errno(&gMysql))
			...
			if(strstr(mysql_error(&gMysql)," doesn't exist"))
                	{
				CreatetStatus();
				{{cProject}}("New tStatus table created");
                	}
			else
			{
...
void NewtStatus(unsigned uMode)
{
	...
	{{cProject}}Log(uStatus,"tStatus","New");
#endif
...
void DeletetStatus(void)
{
	...
	{
#ifdef ISM3FIELDS
		{{cProject}}Log(uStatus,"tStatus","Del");
#endif
	...
	{
#ifdef ISM3FIELDS
		{{cProject}}Log(uStatus,"tStatus","DelError");
#endif
...
void ModtStatus(void)
{
	...
	{{cProject}}Log(uStatus,"tStatus","Mod");
#endif
	tStatus(gcQuery);
