/*
FILE
	tServer source code of {{cProject}}.cgi
	...
	if(!cResult[0])
	{
		...
		if(mysql_errno(&gMysql))
			...
			if(strstr(mysql_error(&gMysql)," doesn't exist"))
                	{
				CreatetServer();
				{{cProject}}("New tServer table created");
                	}
			else
			{
...
void NewtServer(unsigned uMode)
{
	...
	{{cProject}}Log(uServer,"tServer","New");
#endif
...
void DeletetServer(void)
{
	...
	{
#ifdef ISM3FIELDS
		{{cProject}}Log(uServer,"tServer","Del");
#endif
	...
	{
#ifdef ISM3FIELDS
		{{cProject}}Log(uServer,"tServer","DelError");
#endif
...
void ModtServer(void)
{
	...
	{{cProject}}Log(uServer,"tServer","Mod");
#endif
	tServer(gcQuery);
