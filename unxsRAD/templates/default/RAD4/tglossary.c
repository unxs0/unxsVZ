/*
FILE
	tGlossary source code of {{cProject}}.cgi
...
void tGlossary(const char *cResult)
{
	...
	if(!cResult[0])
	{
		...
		if(mysql_errno(&gMysql))
			...
			if(strstr(mysql_error(&gMysql)," doesn't exist"))
                	{
				CreatetGlossary();
				{{cProject}}("New tGlossary table created");
                	}
			else
			{
	...
	//
	OpenFieldSet("tGlossary Record Data",100);
...
void NewtGlossary(unsigned uMode)
{
	...
	{{cProject}}Log(uGlossary,"tGlossary","New");
#endif
...
void DeletetGlossary(void)
{
	...
	{
#ifdef ISM3FIELDS
		{{cProject}}Log(uGlossary,"tGlossary","Del");
#endif
	...
	{
#ifdef ISM3FIELDS
		{{cProject}}Log(uGlossary,"tGlossary","DelError");
#endif
...
void ModtGlossary(void)
{
	...
	sprintf(gcQuery,"SELECT tGlossary.uGlossary,\
				...
				WHERE tGlossary.uGlossary=%u\
				AND tGlossary.uOwner=tClient.uClient\
				AND (tClient.uOwner=%u OR tClient.uClient=%u)"
			,uGlossary,guLoginClient,guLoginClient);
	else
	...
	{{cProject}}Log(uGlossary,"tGlossary","Mod");
#endif
	tGlossary(gcQuery);
