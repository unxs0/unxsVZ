	...
	if(!cResult[0])
	{
		...
		if(mysql_errno(&gMysql))
			...
			if(strstr(mysql_error(&gMysql)," doesn't exist"))
                	{
				CreatetClient();
				{{cProject}}("New tClient table created");
                	}
			else
			{
...
void NewtClient(unsigned uMode)
{
	...
	{{cProject}}Log(uClient,"tClient","New");
#endif
...
void DeletetClient(void)
{
	...
	{
#ifdef ISM3FIELDS
		{{cProject}}Log(uClient,"tClient","Del");
#endif
	...
	{
#ifdef ISM3FIELDS
		{{cProject}}Log(uClient,"tClient","DelError");
#endif
...
void ModtClient(void)
{
	...
	{{cProject}}Log(uClient,"tClient","Mod");
#endif
	tClient(gcQuery);
...
{
	sprintf(gcQuery,"CREATE TABLE IF NOT EXISTS tClient ("
				" uClient INT UNSIGNED PRIMARY KEY AUTO_INCREMENT,"
				...
				" cCode VARCHAR(32) NOT NULL DEFAULT '' )");
	macro_mySQLQueryHTMLError;
