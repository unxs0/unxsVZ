/*
FILE
	tLogType source code of unxsRAD.cgi
	...
	if(!cResult[0])
	{
		...
		if(mysql_errno(&gMysql))
			...
			if(strstr(mysql_error(&gMysql)," doesn't exist"))
                	{
				CreatetLogType();
				unxsRAD("New tLogType table created");
                	}
			else
			{
...
void NewtLogType(unsigned uMode)
{
	...
	unxsRADLog(uLogType,"tLogType","New");
#endif
...
void DeletetLogType(void)
{
	...
	{
#ifdef ISM3FIELDS
		unxsRADLog(uLogType,"tLogType","Del");
#endif
	...
	{
#ifdef ISM3FIELDS
		unxsRADLog(uLogType,"tLogType","DelError");
#endif
...
void ModtLogType(void)
{
	...
	unxsRADLog(uLogType,"tLogType","Mod");
#endif
	tLogType(gcQuery);
