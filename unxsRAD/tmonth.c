/*
FILE
	tMonth source code of unxsRAD.cgi
	...
	if(!cResult[0])
	{
		...
		if(mysql_errno(&gMysql))
			...
			if(strstr(mysql_error(&gMysql)," doesn't exist"))
                	{
				CreatetMonth();
				unxsRAD("New tMonth table created");
                	}
			else
			{
...
void DeletetMonth(void)
{
	...
	{
#ifdef ISM3FIELDS
		unxsRADLog(uMonth,"tMonth","DelError");
#endif
