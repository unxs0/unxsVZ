...
void ProcesstTemplateSetListVars(pentry entries[], int x);
 ...
 //In tTemplateSetfunc.c file included below
	...
	if(!cResult[0])
	{
		...
		if(mysql_errno(&gMysql))
			...
			if(strstr(mysql_error(&gMysql)," doesn't exist"))
                	{
				CreatetTemplateSet();
				unxsRAD("New tTemplateSet table created");
                	}
			else
			{
...
void NewtTemplateSet(unsigned uMode)
{
	...
	unxsRADLog(uTemplateSet,"tTemplateSet","New");
#endif
...
void DeletetTemplateSet(void)
{
	...
	{
#ifdef ISM3FIELDS
		unxsRADLog(uTemplateSet,"tTemplateSet","Del");
#endif
	...
	{
#ifdef ISM3FIELDS
		unxsRADLog(uTemplateSet,"tTemplateSet","DelError");
#endif
...
void ModtTemplateSet(void)
{
	...
	unxsRADLog(uTemplateSet,"tTemplateSet","Mod");
#endif
	tTemplateSet(gcQuery);
