/*
FILE
	tGlossary source code of unxsRAD.cgi
	...
	if(!cResult[0])
	{
		...
		if(mysql_errno(&gMysql))
			...
			if(strstr(mysql_error(&gMysql)," doesn't exist"))
                	{
				CreatetGlossary();
				unxsRAD("New tGlossary table created");
                	}
			else
			{
...
void NewtGlossary(unsigned uMode)
{
	...
	unxsRADLog(uGlossary,"tGlossary","New");
#endif
...
void DeletetGlossary(void)
{
	...
	{
#ifdef ISM3FIELDS
		unxsRADLog(uGlossary,"tGlossary","Del");
#endif
	...
	{
#ifdef ISM3FIELDS
		unxsRADLog(uGlossary,"tGlossary","DelError");
#endif
...
void ModtGlossary(void)
{
	...
	unxsRADLog(uGlossary,"tGlossary","Mod");
#endif
	tGlossary(gcQuery);
