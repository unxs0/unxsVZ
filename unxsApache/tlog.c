...
FILE
	...
	$Id: tlog.c 2355 2008-12-29 21:44:56Z hus-admin $
PURPOSE
	...
	if(!cResult[0])
	{
		...
		if(mysql_errno(&gMysql))
			...
			if(strstr(mysql_error(&gMysql)," doesn't exist"))
                	{
				CreatetLog();
				mysqlApache2("New tLog table created");
                	}
			else
			{
...
void DeletetLog(void)
{
	...
	{
#ifdef ISM3FIELDS
		mysqlApache2Log(uLog,"tLog","DelError");
#endif
