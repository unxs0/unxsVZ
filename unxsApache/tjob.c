/*
FILE
	tJob source code of mysqlApache2.cgi
	Built by mysqlRAD2.cgi (C) Gary Wallis and Hugo Urquiza 2001-2009
	$Id: tjob.c 2355 2008-12-29 21:44:56Z hus-admin $
PURPOSE
...
//uJobSite: Site ID
static unsigned uJobSite=0;
//uJobSiteUser: Site User ID
static unsigned uJobSiteUser=0;
...
#define VAR_LIST_tJob "tJob.uJob,tJob.cLabel,tJob.cServer,tJob.cJobName,tJob.uJobSite,tJob.uJobSiteUser,tJob.cJobData,tJob.uJobDate,tJob.uJobStatus,tJob.cRemoteMsg,tJob.uOwner,tJob.uCreatedBy,tJob.uCreatedDate,tJob.uModBy,tJob.uModDate"
...
void ProcesstJobVars(pentry entries[], int x)
{
	...
	for(i=0;i<x;i++)
	{
		...
		else if(!strcmp(entries[i].name,"uJobSite"))
			sscanf(entries[i].val,"%u",&uJobSite);
		else if(!strcmp(entries[i].name,"uJobSiteUser"))
			sscanf(entries[i].val,"%u",&uJobSiteUser);
	...
	if(!cResult[0])
	{
		...
		if(mysql_errno(&gMysql))
			...
			if(strstr(mysql_error(&gMysql)," doesn't exist"))
                	{
				CreatetJob();
				mysqlApache2("New tJob table created");
                	}
			else
			{
		...
		sscanf(field[4],"%u",&uJobSite);
		sscanf(field[5],"%u",&uJobSiteUser);
		cJobData=field[6];
...
//uJobSite
	OpenRow(LANG_FL_tJob_uJobSite,"black");
	if(guPermLevel>=0 && uMode)
	{
	printf("%s<input type=hidden name=uJobSite value=%u >\n",ForeignKey("tSite","cDomain",uJobSite),uJobSite);
	}
	else
	{
	printf("%s<input type=hidden name=uJobSite value=%u >\n",ForeignKey("tSite","cDomain",uJobSite),uJobSite);
...
//uJobSiteUser
	OpenRow(LANG_FL_tJob_uJobSiteUser,"black");
	if(guPermLevel>=0 && uMode)
	{
	printf("%s<input type=hidden name=uJobSiteUser value=%u >\n",ForeignKey("tSiteUser","cLogin",uJobSiteUser),uJobSiteUser);
	}
	else
	{
	printf("%s<input type=hidden name=uJobSiteUser value=%u >\n",ForeignKey("tSiteUser","cLogin",uJobSiteUser),uJobSiteUser);
...
void DeletetJob(void)
{
	...
	{
#ifdef ISM3FIELDS
		mysqlApache2Log(uJob,"tJob","DelError");
#endif
...
void Insert_tJob(void)
{
	...
	//insert query
	sprintf(gcQuery,"INSERT INTO tJob SET uJob=%u,cLabel='%s',cServer='%s',cJobName='%s',uJobSite=%u,uJobSiteUser=%u,cJobData='%s',uJobDate=%lu,uJobStatus=%u,cRemoteMsg='%s',uOwner=%u,uCreatedBy=%u,uCreatedDate=UNIX_TIMESTAMP(NOW())",
			uJob
			,TextAreaSave(cLabel)
...
void Update_tJob(char *cRowid)
{
	...
	//update query
	sprintf(gcQuery,"UPDATE tJob SET uJob=%u,cLabel='%s',cServer='%s',cJobName='%s',uJobSite=%u,uJobSiteUser=%u,cJobData='%s',uJobDate=%lu,uJobStatus=%u,cRemoteMsg='%s',uModBy=%u,uModDate=UNIX_TIMESTAMP(NOW()) WHERE _rowid=%s",
			uJob
			,TextAreaSave(cLabel)
			,TextAreaSave(cServer)
			,TextAreaSave(cJobName)
...
void tJobList(void)
{
	...
	for(guN=0;guN<(guEnd-guStart+1);guN++)
	{
		...
		printf("<td><input type=submit name=ED%s value=Edit> %s<td>%s<td>%s<td>%s<td>%s<td>%s<td><textarea disabled>%s</textarea><td>%s<td>%s<td>%s<td>%s<td>%s<td>%s<td>%s<td>%s</tr>"
			...
			,field[2]
			,field[3]
			,ForeignKey("tSite","cDomain",strtoul(field[4],NULL,10))
			,ForeignKey("tSiteUser","cLogin",strtoul(field[5],NULL,10))
			,field[6]
			,cBuf7
...
{
	sprintf(gcQuery,"CREATE TABLE IF NOT EXISTS tJob ( cJobName VARCHAR(64) NOT NULL DEFAULT '', uModDate INT UNSIGNED NOT NULL DEFAULT 0, cServer VARCHAR(64) NOT NULL DEFAULT '', uCreatedDate INT UNSIGNED NOT NULL DEFAULT 0, uModBy INT UNSIGNED NOT NULL DEFAULT 0, uOwner INT UNSIGNED NOT NULL DEFAULT 0,index (uOwner), uCreatedBy INT UNSIGNED NOT NULL DEFAULT 0, cJobData TEXT NOT NULL DEFAULT '', uJob INT UNSIGNED PRIMARY KEY AUTO_INCREMENT, cLabel VARCHAR(100) NOT NULL DEFAULT '', uJobStatus INT UNSIGNED NOT NULL DEFAULT 0, uJobDate INT UNSIGNED NOT NULL DEFAULT 0, cRemoteMsg VARCHAR(32) NOT NULL DEFAULT '', uJobSite INT UNSIGNED NOT NULL DEFAULT 0, uJobSiteUser INT UNSIGNED NOT NULL DEFAULT 0 )");
	mysql_query(&gMysql,gcQuery);
	if(mysql_errno(&gMysql))
		htmlPlainTextError(mysql_error(&gMysql));
}//CreatetJob()
