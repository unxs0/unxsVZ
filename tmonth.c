/*
FILE
	tMonth source code of unxsVZ.cgi
	Built by mysqlRAD2.cgi (C) Gary Wallis 2001-2007
PURPOSE
	Month table for archives
AUTHOR/LEGAL
	...
	if(!cResult[0])
	{
		...
		if(mysql_errno(&gMysql))
			...
			if(strstr(mysql_error(&gMysql)," doesn't exist"))
                	{
				CreatetMonth();
				unxsVZ("New tMonth table created");
                	}
			else
			{
...
void DeletetMonth(void)
{
	...
	{
#ifdef ISM3FIELDS
		unxsVZLog(uMonth,"tMonth","DelError");
#endif
...
void ModtMonth(void)
{
	...
	sprintf(gcQuery,"SELECT tMonth.uMonth,\
				tMonth.uModDate\
				FROM tMonth,tClient\
				WHERE tMonth.uMonth=%u\
				AND tMonth.uOwner=tClient.uClient\
				AND (tClient.uOwner=%u OR tClient.uClient=%u)"
			,uMonth,guLoginClient,guLoginClient);
	else
	sprintf(gcQuery,"SELECT uMonth,uModDate FROM tMonth\
				WHERE uMonth=%u"
						,uMonth);
#else
	sprintf(gcQuery,"SELECT uMonth FROM tMonth\
...
void tMonthList(void)
{
	...
	for(guN=0;guN<(guEnd-guStart+1);guN++)
	{
		...
		if(luTime6)
			...
			sprintf(cBuf6,"---");
		printf("<td><a class=darkLink href=unxsVZ.cgi?gcFunction=tMonth&uMonth=%s>%s</a>"
			,field[0]
			,field[0]);
...
{
	sprintf(gcQuery,"CREATE TABLE IF NOT EXISTS tMonth ("
