...
//exit(0);
	...
	if(gcFunction[0])
	{
		...
		{
		printf("Set-Cookie: ispCRMLogin=; expires=\"Mon, 01-Jan-1971 00:10:10 GMT\"\n");
		printf("Set-Cookie: ispCRMPasswd=; expires=\"Mon, 01-Jan-1971 00:10:10 GMT\"\n");
...
void htmlPlainTextError(const char *cError)
{
	...
	printf("Content-type: text/plain\n\n");
	printf("Please report this ispCRM fatal error ASAP:\n%s\n",cError);
...
void SSLCookieLogin(void)
{
	...
	if((ptr=strstr(gcCookie,"ispCRMLogin=")))
	{
		ptr+=strlen("ispCRMLogin=");
		if((ptr2=strchr(ptr,';')))
		{
			*ptr2=0;
	...
	if((ptr=strstr(gcCookie,"ispCRMPasswd=")))
	{
		ptr+=strlen("ispCRMPasswd=");
		if((ptr2=strchr(ptr,';')))
		{
			*ptr2=0;
...
void SetLogin(void)
{
	if( iValidLogin(0) )
	{
		printf("Set-Cookie: ispCRMLogin=%s;\n",gcLogin);
		printf("Set-Cookie: ispCRMPasswd=%s;\n",gcPasswd);
		sprintf(gcUser,"%.41s",gcLogin);
		guSSLCookieLogin=1;
		GetPLAndClient(gcUser);	
