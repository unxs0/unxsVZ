...
//exit(0);
	...
	if(gcFunction[0])
	{
		...
		{
		printf("Set-Cookie: ispHelpLogin=; expires=\"Mon, 01-Jan-1971 00:10:10 GMT\"\n");
		printf("Set-Cookie: ispHelpPasswd=; expires=\"Mon, 01-Jan-1971 00:10:10 GMT\"\n");
...
void htmlPlainTextError(const char *cError)
{
	...
	printf("Content-type: text/plain\n\n");
	printf("Please report this ispHelp fatal error ASAP:\n%s\n",cError);
...
{
	if(!strcmp(cFunction,"funcTicketNavList"))
...
void SSLCookieLogin(void)
{
	...
	if((ptr=strstr(gcCookie,"ispHelpLogin=")))
	{
		ptr+=strlen("ispHelpLogin=");
		if((ptr2=strchr(ptr,';')))
		{
			*ptr2=0;
	...
	if((ptr=strstr(gcCookie,"ispHelpPasswd=")))
	{
		ptr+=strlen("ispHelpPasswd=");
		if((ptr2=strchr(ptr,';')))
		{
			*ptr2=0;
...
void SetLogin(void)
{
	if( iValidLogin(0) )
	{
		printf("Set-Cookie: ispHelpLogin=%s;\n",gcLogin);
		printf("Set-Cookie: ispHelpPasswd=%s;\n",gcPasswd);
		sprintf(gcUser,"%.41s",gcLogin);
		guSSLCookieLogin=1;
		GetPLAndClient(gcUser);	
