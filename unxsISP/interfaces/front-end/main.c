...
//exit(0);
	...
	if(gcFunction[0])
	{
		...
		{
		printf("Set-Cookie: riedelRegLogin=; expires=\"Mon, 01-Jan-1971 00:10:10 GMT\"\n");
		printf("Set-Cookie: riedelRegPasswd=; expires=\"Mon, 01-Jan-1971 00:10:10 GMT\"\n");
...
void htmlPlainTextError(const char *cError)
{
	...
	printf("Content-type: text/plain\n\n");
	printf("Please report this riedelReg fatal error ASAP:\n%s\n",cError);
...
void htmlFooter(char *cTemplateName)
{
	if(cTemplateName[0])
	{
		...
		if((field=mysql_fetch_row(res)))
		{
			...
			template.cpValue[2]="&copy; 2009 Riedel GmbH. All Rights Reserved.";
			template.cpName[3]="";
...
void ConnectDb(void)
{
		...
		if (!mysql_real_connect(&gMysql,DBIP1,DBLOGIN,DBPASSWD,DBNAME,DBPORT,DBSOCKET,0))
		{
			...
			printf("Debug info: %s (%s,%s)\n",mysql_error(&gMysql),DBIP0,DBIP1);
			exit(0);
		}
        }
...
void AppFunctions(FILE *fp,char *cFunction)
{
	...
	else if(!strcmp(cFunction,"funcSelectVPN"))
		funcSelectVPN(fp);
	else if(!strcmp(cFunction,"funcRegTerms"))
		funcRegTerms(fp);
...
void SSLCookieLogin(void)
{
	...
	if((ptr=strstr(gcCookie,"riedelRegLogin=")))
	{
		ptr+=strlen("riedelRegLogin=");
		if((ptr2=strchr(ptr,';')))
		{
			*ptr2=0;
	...
	if((ptr=strstr(gcCookie,"riedelRegPasswd=")))
	{
		ptr+=strlen("riedelRegPasswd=");
		if((ptr2=strchr(ptr,';')))
		{
			*ptr2=0;
...
void SetLogin(void)
{
	if( iValidLogin(0) )
	{
		printf("Set-Cookie: riedelRegLogin=%s;\n",gcLogin);
		printf("Set-Cookie: riedelRegPasswd=%s;\n",gcPasswd);
		sprintf(gcUser,"%.41s",gcLogin);
		guSSLCookieLogin=1;
		GetPLAndClient(gcUser);	
...
void funcDisplayDashBoard(FILE *fp)
{
	...
	*/
	...
	sprintf(gcQuery,"SELECT tJob.cJobName,GREATEST(tJob.uCreatedDate,tJob.uModDate),tClient.cLabel,cServer FROM tJob,tClient WHERE tJob.uJobClient=tClient.uClient AND tJob.uJobStatus!=%u ORDER BY GREATEST(tJob.uCreatedDate,tJob.uModDate) DESC LIMIT 20",mysqlISP_Deployed);
	mysql_query(&gMysql,gcQuery);
	if(mysql_errno(&gMysql))
	{
		fprintf(fp,"%s\n",mysql_error(&gMysql));
