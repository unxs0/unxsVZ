...
void htmlLogin(void)
{
	...
	htmlLoginPage("","CustomerLogin.Body");
	htmlFooter("Footer");
...
void htmlPlainTextError(const char *cError)
{
	...
	printf("Content-type: text/plain\n\n");
	printf("Please report this ispClient fatal error ASAP:\n%s\n",cError);
...
{
	if(!strcmp(cFunction,"funcSelectExpMonth"))
...
void SSLCookieLogin(void)
{
	...
	if((ptr=strstr(gcCookie,"ispClientLogin=")))
	{
		ptr+=strlen("ispClientLogin=");
		if((ptr2=strchr(ptr,';')))
		{
			*ptr2=0;
	...
	if((ptr=strstr(gcCookie,"ispClientPasswd=")))
	{
		ptr+=strlen("ispClientPasswd=");
		if((ptr2=strchr(ptr,';')))
		{
			*ptr2=0;
...
void SetLogin(void)
{
	if( iValidLogin(0) )
	{
		printf("Set-Cookie: ispClientLogin=%s;\n",gcLogin);
		printf("Set-Cookie: ispClientPasswd=%s;\n",gcPasswd);
		sprintf(gcUser,"%.41s",gcLogin);
		guSSLCookieLogin=1;
		GetPLAndClient(gcUser);	
