/*
FILE
	tLogType source code of mysqlISP2.cgi
	...
	if(!cResult[0])
	{
		...
		if(mysql_errno(&gMysql))
			...
			if(strstr(mysql_error(&gMysql)," doesn't exist"))
                	{
				CreatetLogType();
				mysqlISP2("New tLogType table created");
                	}
			else
			{
...
void NewtLogType(unsigned uMode)
{
	...
	mysqlISP2Log(uLogType,"tLogType","New");
#endif
...
void DeletetLogType(void)
{
	...
	{
#ifdef ISM3FIELDS
		mysqlISP2Log(uLogType,"tLogType","Del");
#endif
	...
	{
#ifdef ISM3FIELDS
		mysqlISP2Log(uLogType,"tLogType","DelError");
#endif
...
void ModtLogType(void)
{
	...
	mysqlISP2Log(uLogType,"tLogType","Mod");
#endif
	tLogType(gcQuery);
