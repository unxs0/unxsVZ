/*
FILE
	tLogType source code of unxsISP.cgi
	...
	if(!cResult[0])
	{
		...
		if(mysql_errno(&gMysql))
			...
			if(strstr(mysql_error(&gMysql)," doesn't exist"))
                	{
				CreatetLogType();
				unxsISP("New tLogType table created");
                	}
			else
			{
...
void NewtLogType(unsigned uMode)
{
	...
	unxsISPLog(uLogType,"tLogType","New");
#endif
...
void DeletetLogType(void)
{
	...
	{
#ifdef ISM3FIELDS
		unxsISPLog(uLogType,"tLogType","Del");
#endif
	...
	{
#ifdef ISM3FIELDS
		unxsISPLog(uLogType,"tLogType","DelError");
#endif
...
void ModtLogType(void)
{
	...
	unxsISPLog(uLogType,"tLogType","Mod");
#endif
	tLogType(gcQuery);
