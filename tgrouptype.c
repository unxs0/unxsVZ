	...
	if(!cResult[0])
	{
		...
		if(mysql_errno(&gMysql))
			...
			if(strstr(mysql_error(&gMysql)," doesn't exist"))
                	{
				CreatetGroupType();
				unxsVZ("New tGroupType table created");
                	}
			else
			{
...
void NewtGroupType(unsigned uMode)
{
	...
	unxsVZLog(uGroupType,"tGroupType","New");
#endif
...
void DeletetGroupType(void)
{
	...
	{
#ifdef ISM3FIELDS
		unxsVZLog(uGroupType,"tGroupType","Del");
#endif
	...
	{
#ifdef ISM3FIELDS
		unxsVZLog(uGroupType,"tGroupType","DelError");
#endif
...
void ModtGroupType(void)
{
	...
	unxsVZLog(uGroupType,"tGroupType","Mod");
#endif
	tGroupType(gcQuery);
...
void tGroupTypeList(void)
{
	...
	for(guN=0;guN<(guEnd-guStart+1);guN++)
	{
		...
		if(luTime6)
			...
			sprintf(cBuf6,"---");
		printf("<td><a class=darkLink href=unxsVZ.cgi?gcFunction=tGroupType&uGroupType=%s>%s</a>"
			,field[0]
			,field[0]);
