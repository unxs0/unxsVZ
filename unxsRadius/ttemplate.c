...
{
	sprintf(gcQuery,"DELETE FROM tTemplate WHERE uTemplate=%u",uTemplate);
...
void Insert_tTemplate(void)
{
	sprintf(gcQuery,"INSERT INTO tTemplate SET uTemplate=%u,cLabel='%s',uTemplateSet=%u,uTemplateType=%u,"
			"cComment='%s',cTemplate='%s',uOwner=%u,uCreatedBy=%u,uCreatedDate=UNIX_TIMESTAMP(NOW())",
				uTemplate
				,TextAreaSave(cLabel)
				,uTemplateSet
				,uTemplateType
...
void Update_tTemplate(char *cRowid)
{
	sprintf(gcQuery,"UPDATE tTemplate SET uTemplate=%u,cLabel='%s',uTemplateSet=%u,uTemplateType=%u,"
			"cComment='%s',cTemplate='%s',uModBy=%u,uModDate=UNIX_TIMESTAMP(NOW()) WHERE _rowid=%s",
				uTemplate
				,TextAreaSave(cLabel)
				,uTemplateSet
				,uTemplateType
