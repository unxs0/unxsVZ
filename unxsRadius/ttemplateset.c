/*
FILE
	tTemplateSet source code of mysqlRadius2.cgi
	Built by mysqlRAD2.cgi (C) Gary Wallis 2001-2007
	$Id: ttemplateset.c 1335 2007-07-23 23:15:44Z Gary $
PURPOSE
	...
	if(!cResult[0])
	{
		...
		if(mysql_errno(&gMysql))
			...
			if(strstr(mysql_error(&gMysql)," doesn't exist"))
                	{
				CreatetTemplateSet();
				mysqlRadius2("New tTemplateSet table created");
                	}
			else
			{
...
void NewtTemplateSet(unsigned uMode)
{
	...
	sprintf(gcQuery,"SELECT uTemplateSet FROM tTemplateSet WHERE uTemplateSet=%u",uTemplateSet);
	...
	mysqlRadius2Log(uTemplateSet,"tTemplateSet","New");
...
{
	sprintf(gcQuery,"DELETE FROM tTemplateSet WHERE uTemplateSet=%u",uTemplateSet);
	macro_mySQLQueryHTMLError;

	//tTemplateSet("Record Deleted");
	if(mysql_affected_rows(&gMysql)>0)
	{
		mysqlRadius2Log(uTemplateSet,"tTemplateSet","Del");
		tTemplateSet(LANG_NBR_RECDELETED);
	}
	else
	{
		mysqlRadius2Log(uTemplateSet,"tTemplateSet","DelError");
...
void Insert_tTemplateSet(void)
{
	sprintf(gcQuery,"INSERT INTO tTemplateSet SET uTemplateSet=%u,cLabel='%s',uOwner=%u,"
...
void Update_tTemplateSet(char *cRowid)
{
	sprintf(gcQuery,"UPDATE tTemplateSet SET uTemplateSet=%u,cLabel='%s',uModBy=%u,"
...
void ModtTemplateSet(void)
{
	...
	//Mod select gcQuery
	sprintf(gcQuery,"SELECT uTemplateSet,uModDate FROM tTemplateSet WHERE uTemplateSet=%u",uTemplateSet);
	macro_mySQLRunAndStore(res);
	i=mysql_num_rows(res);
	...
	mysqlRadius2Log(uTemplateSet,"tTemplateSet","Mod");
	tTemplateSet(gcQuery);
