...
void NewtMonth(unsigned uMode)
{
	...
	sprintf(gcQuery,"SELECT uMonth FROM tMonth WHERE uMonth=%u",uMonth);
...
{
	sprintf(gcQuery,"DELETE FROM tMonth WHERE uMonth=%u",uMonth);
...
void Insert_tMonth(void)
{
	sprintf(gcQuery,"INSERT INTO tMonth SET uMonth=%u,cLabel='%s',uOwner=%u,uCreatedBy=%u,"
...
void Update_tMonth(char *cRowid)
{
	sprintf(gcQuery,"UPDATE tMonth SET uMonth=%u,cLabel='%s',uModBy=%u,"
