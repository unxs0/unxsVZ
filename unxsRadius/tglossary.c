...
{
	sprintf(gcQuery,"DELETE FROM tGlossary WHERE uGlossary=%u",uGlossary);
...
void Insert_tGlossary(void)
{
	sprintf(gcQuery,"INSERT INTO tGlossary SET uGlossary=%u,cLabel='%s',cText='%s',uOwner=%u,"
...
void Update_tGlossary(char *cRowid)
{
	sprintf(gcQuery,"UPDATE tGlossary SET uGlossary=%u,cLabel='%s',cText='%s',uModBy=%u,"
...
void ModtGlossary(void)
{
	...
	sprintf(gcQuery,"SELECT uGlossary,uModDate FROM tGlossary WHERE uGlossary=%u",uGlossary);
	macro_mySQLRunAndStore(res);
	i=mysql_num_rows(res);
