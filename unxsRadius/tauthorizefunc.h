	...
	if(!strcmp(gcFunction,"tAuthorizeTools"))
	{
		if(!strcmp(gcCommand,LANG_NB_NEW))
			...
			if(guPermLevel>=12)
			{
                ...
                {
                        ProcesstAuthorizeVars(entries,x);
			if(uAllowMod(uOwner,uCreatedBy) && (uPerm<=guPermLevel) )
			{
				guMode=2002;
			...
			if(uAllowMod(uOwner,uCreatedBy) && (uPerm<=guPermLevel) )
			{
				...
				//Only modify passwd if presented a clear text one.
				if(cClrPasswd[0])
				{
	...
	switch(guMode)
		...
		default:
			...
			if(uCertClient>1 && uOwner>1)
				printf("This login appears to belong to a <a class=darkLink href=mysqlRadius2.cgi?gcFunction=tClient&uClient=%u>'%s'</a> company contact '<a class=darkLink href=unxsVZ.cgi?gcFunction=tClient&uClient=%u>%s</a>'.<br>The uPerm corresponds to permission level '%s'.",uOwner,ForeignKey("tClient","cLabel",uOwner),uCertClient,ForeignKey("tClient","cLabel",uCertClient),cUserLevel(uPerm));
			else if(uOwner>1)
				printf("This login appears to belong to a '<a class=darkLink href=mysqlRadius2.cgi?gcFunction=tClient&uClient=%u>%s</a>' company contact, but that has been root aliased to usually run the back-office with complete permissions. <br>The uPerm corresponds to permission level '%s'.",uOwner,ForeignKey("tClient","cLabel",uOwner),cUserLevel(uPerm));
			printf("<p>\n");
			tAuthorizeNavList();
	}
...
void tAuthorizeNavList(void)
{
	...
	if(mysql_num_rows(res))
	{
			...
			printf("<a class=darkLink href=mysqlRadius2.cgi?gcFunction=tAuthorize&uAuthorize=%s>"
