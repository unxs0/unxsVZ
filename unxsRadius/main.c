...
int main(int iArgc, char *cArgv[])
{
	...
	if(strcmp(getenv("REQUEST_METHOD"),"POST"))
	{
		...
		if(gcFunction[0])
		{
			...
			else if(!strcmp(gcFunction,"tProfile"))
				ExttProfileGetHook(gentries,x);
			else if(!strcmp(gcFunction,"tProfileName"))
				ExttProfileNameGetHook(gentries,x);
			else if(!strcmp(gcFunction,"tServer"))
				ExttServerGetHook(gentries,x);
			else if(!strcmp(gcFunction,"tNAS"))
				ExttNASGetHook(gentries,x);
			...
			else if(!strcmp(gcFunction,"tNASGroup"))
				ExttNASGroupGetHook(gentries,x);
...
void Header_ism3(char *title, int js)
{
	...
	if(!strcmp(gcFunction,"tUser") || !strcmp(gcFunction,"tUserTools") ||
		...
		ExttUserNavBar();
	else if(!strcmp(gcFunction,"tProfile") || !strcmp(gcFunction,"tProfileTools") ||
			!strcmp(gcFunction,"tProfileList"))
		ExttProfileNavBar();
	else if(!strcmp(gcFunction,"tProfileName") || !strcmp(gcFunction,"tProfileNameTools") ||
			!strcmp(gcFunction,"tProfileNameList"))
		ExttProfileNameNavBar();
	else if(!strcmp(gcFunction,"tServer") || !strcmp(gcFunction,"tServerTools") ||
			!strcmp(gcFunction,"tServerList"))
		ExttServerNavBar();
	else if(!strcmp(gcFunction,"tNAS") || !strcmp(gcFunction,"tNASTools") ||
			!strcmp(gcFunction,"tNASList"))
		ExttNASNavBar();
	...
	else if(!strcmp(gcFunction,"tNASGroup") || !strcmp(gcFunction,"tNASGroupTools") ||
			!strcmp(gcFunction,"tNASGroupList"))
		ExttNASGroupNavBar();
	...
	{
	  printf("\t\t\t<li");
	  if(strcmp(gcFunction,"tProfile") && strcmp(gcFunction,"tProfileTools") &&
			strcmp(gcFunction,"tProfileList"))
		  printf(">\n");
	  else
		  printf(" id=current>\n");
	...
	{
	  printf("\t\t\t<li");
	  if(strcmp(gcFunction,"tProfileName") && strcmp(gcFunction,"tProfileNameTools") &&
			strcmp(gcFunction,"tProfileNameList"))
		  printf(">\n");
	  else
		  printf(" id=current>\n");
	...
	{
	  printf("\t\t\t<li");
	  if(strcmp(gcFunction,"tNAS") && strcmp(gcFunction,"tNASTools") &&
			strcmp(gcFunction,"tNASList"))
		  printf(">\n");
	  else
		  printf(" id=current>\n");
	...
	{
	  printf("\t\t\t<li");
	  if(strcmp(gcFunction,"tNASGroup") && strcmp(gcFunction,"tNASGroupTools") &&
			strcmp(gcFunction,"tNASGroupList"))
		  printf(">\n");
	  else
		  printf(" id=current>\n");
...
/*
	//Use cLabel as a temp storage var ;)
	sprintf(cLocalFieldName,"%1$s.%2$s",cLocalTableName,cFieldName);
	sprintf(cLabel,"%1$s._rowid,%1$s.%2$s",cLocalTableName,cFieldName);
	ExtSelectSearch(cLocalTableName,cLabel,cLocalFieldName,"",NULL,0);
        mysql_query(&gMysql,gcQuery);
...
*/
	...
	//This is not an owner pull down!
        sprintf(gcQuery,"SELECT _rowid,%s FROM %s ORDER BY %s",
                                cFieldName,cLocalTableName,cOrderby);
	macro_mySQLRunAndStoreTextVoidRet(mysqlRes);
	i=mysql_num_rows(mysqlRes);
...
void SetLogin(void)
{
	if( iValidLogin(0) )
	{
		...
		guSSLCookieLogin=1;
		sprintf(gcQuery,"INSERT INTO tLog SET cLabel='login ok %.99s',uLogType=6,uPermLevel=%u,uLoginClient=%u,"
				...
				const char *cSearch,const char *cExtraCond,unsigned uMaxResults)
{
	...
	else
	{
		if(cExtraCond!=NULL)
			sprintf(gcQuery,"SELECT %1$s FROM %3$s," TCLIENT
				 	" WHERE %4$s LIKE '%5$s%%' AND %6$s AND %7$s.uOwner=" TCLIENT ".uClient"
					" AND (" TCLIENT ".uClient=%2$u OR " TCLIENT ".uOwner"
					" IN (SELECT uClient FROM " TCLIENT " WHERE uOwner=%2$u OR uClient=%2$u))"
					" ORDER BY %4$s",
						cVarList,guCompany,
						cTable,cSearchField,cSearch,cExtraCond,cTableCopy);
		else
			sprintf(gcQuery,"SELECT %1$s FROM %3$s," TCLIENT
				 	" WHERE %4$s LIKE '%5$s%%' AND  %6$s.uOwner=" TCLIENT ".uClient"
					" AND (" TCLIENT ".uClient=%2$u OR " TCLIENT ".uOwner"
					" IN (SELECT uClient FROM " TCLIENT " WHERE uOwner=%2$u OR uClient=%2$u))"
					" ORDER BY %4$s",
						cVarList,guCompany,
						cTable,cSearchField,cSearch,cTableCopy);
	}
