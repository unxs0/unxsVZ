...
void NewtServer(unsigned uMode)
{
	...
	sprintf(gcQuery,"SELECT uServer FROM tServer WHERE uServer=%u",uServer);
...
{
	sprintf(gcQuery,"DELETE FROM tServer WHERE uServer=%u",uServer);
...
void Insert_tServer(void)
{
	sprintf(gcQuery,"INSERT INTO tServer SET uServer=%u,cLabel='%s',uOwner=%u,uCreatedBy=%u,"
...
void Update_tServer(char *cRowid)
{
	sprintf(gcQuery,"UPDATE tServer SET uServer=%u,cLabel='%s',uModBy=%u,"
...
void ModtServer(void)
{
	...
	sprintf(gcQuery,"SELECT uServer,uModDate FROM tServer WHERE uServer=%u",uServer);
	macro_mySQLRunAndStore(res);
	i=mysql_num_rows(res);
