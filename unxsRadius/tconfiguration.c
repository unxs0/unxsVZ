...
{
	sprintf(gcQuery,"DELETE FROM tConfiguration WHERE uConfiguration=%u",uConfiguration);
...
void Insert_tConfiguration(void)
{
	sprintf(gcQuery,"INSERT INTO tConfiguration SET uConfiguration=%u,uServer=%u,cLabel='%s',"
			"cValue='%s',cComment='%s',uOwner=%u,uCreatedBy=%u,uCreatedDate=UNIX_TIMESTAMP(NOW())",
				uConfiguration
				,uServer
				,TextAreaSave(cLabel)
				,TextAreaSave(cValue)
...
void Update_tConfiguration(char *cRowid)
{
	sprintf(gcQuery,"UPDATE tConfiguration SET uConfiguration=%u,uServer=%u,cLabel='%s',cValue='%s',"
			"cComment='%s',uModBy=%u,uModDate=UNIX_TIMESTAMP(NOW()) WHERE _rowid=%s",
				uConfiguration
				,uServer
				,TextAreaSave(cLabel)
				,TextAreaSave(cValue)
