...
PURPOSE
	...
	Program app functionality in tradacctmonthfunc.h while 
...
#define VAR_LIST_tRadacctMonth "tRadacctMonth.uRadacct,tRadacctMonth.cLogin,tRadacctMonth.cSessionID,tRadacctMonth.cNAS,tRadacctMonth.uConnectTime,tRadacctMonth.uPort,tRadacctMonth.uPortType,tRadacctMonth.uService,tRadacctMonth.uProtocol,tRadacctMonth.cIP,tRadacctMonth.cLine,tRadacctMonth.cCallerID,tRadacctMonth.uInBytes,tRadacctMonth.uOutBytes,tRadacctMonth.uTermCause,tRadacctMonth.uStartTime,tRadacctMonth.uStopTime,tRadacctMonth.cInfo"

 //Local only
void Insert_tRadacctMonth(void);
void Update_tRadacctMonth(char *cRowid);
void ProcesstRadacctMonthListVars(pentry entries[], int x);

 //In tRadacctMonthfunc.h file included below
...
#include "tradacctmonthfunc.h"

 //Table Variables Assignment Function
void ProcesstRadacctMonthVars(pentry entries[], int x)
{
	register int i;
	...
	for(i=0;i<x;i++)
	{
		if(!strcmp(entries[i].name,"uRadacct"))
			...
			sprintf(cLogin,"%.99s",entries[i].val);
...
int tRadacctMonthCommands(pentry entries[], int x)
{
	ProcessControlVars(entries,x);

	ExttRadacctMonthCommands(entries,x);
	...
	if(!strcmp(gcFunction,"tRadacctMonthTools"))
	{
		...
		//Default
		ProcesstRadacctMonthVars(entries,x);
		tRadacctMonth("");
	}
	else if(!strcmp(gcFunction,"tRadacctMonthList"))
	{
		ProcessControlVars(entries,x);
		ProcesstRadacctMonthListVars(entries,x);
		tRadacctMonthList();
	}

	return(0);

}//tRadacctMonthCommands()


void tRadacctMonth(const char *cResult)
{
	MYSQL_RES *res;
	MYSQL_RES *res2;
	MYSQL_ROW field;
	...
	{
		if(guMode)
			ExttRadacctMonthSelectRow();
		else
			ExttRadacctMonthSelect();
		...
		if((guI=mysql_num_rows(res)))
		{
			if(guMode==6)
			{
			sprintf(gcQuery,"SELECT _rowid FROM tRadacctMonth WHERE uRadacct=%u"
						,uRadacct);
				mysql_query(&gMysql,gcQuery);
	...
	Header_ism3(":: tRadacctMonth",1);
	printf("<table width=100%% cellspacing=0 cellpadding=0>\n");
	printf("<tr><td colspan=2 align=right valign=center>");


	printf("<input type=hidden name=gcFunction value=tRadacctMonthTools>");
	...
	printf("<tr><td valign=top width=25%%>");
        ...
        ExttRadacctMonthButtons();

        printf("</td><td valign=top>");
	//
	OpenFieldSet("tRadacctMonth Record Data",100);

	if(guMode==2000 || guMode==2002)
		tRadacctMonthInput(1);
	else
		tRadacctMonthInput(0);
	...
	printf("<tr><td colspan=2>");
        ExttRadacctMonthAuxTable();

	Footer_ism3();

}//end of tRadacctMonth();


void tRadacctMonthInput(unsigned uMode)
{

//uRadacct
	OpenRow(LANG_FL_tRadacctMonth_uRadacct,"black");
	printf("<input title='%s' type=text name=uRadacct value=%u size=16 maxlength=11 "
,LANG_FT_tRadacctMonth_uRadacct,uRadacct);
...
//cLogin
	OpenRow(LANG_FL_tRadacctMonth_cLogin,"black");
	printf("<input title='%s' type=text name=cLogin value=\"%s\" size=40 maxlength=99 "
,LANG_FT_tRadacctMonth_cLogin,EncodeDoubleQuotes(cLogin));
	if(guPermLevel>=20 && uMode)
	{
		printf("></td></tr>\n");
	}
...
//cSessionID
	OpenRow(LANG_FL_tRadacctMonth_cSessionID,"black");
	printf("<input title='%s' type=text name=cSessionID value=\"%s\" size=40 maxlength=32 "
,LANG_FT_tRadacctMonth_cSessionID,EncodeDoubleQuotes(cSessionID));
	if(guPermLevel>=20 && uMode)
	{
		printf("></td></tr>\n");
	}
...
//cNAS
	OpenRow(LANG_FL_tRadacctMonth_cNAS,"black");
	printf("<input title='%s' type=text name=cNAS value=\"%s\" size=40 maxlength=32 "
,LANG_FT_tRadacctMonth_cNAS,EncodeDoubleQuotes(cNAS));
	if(guPermLevel>=20 && uMode)
	{
		printf("></td></tr>\n");
	}
...
//uConnectTime
	OpenRow(LANG_FL_tRadacctMonth_uConnectTime,"black");
	printf("<input title='%s' type=text name=uConnectTime value=%lu size=20 maxlength=11 "
,LANG_FT_tRadacctMonth_uConnectTime,uConnectTime);
...
//uPort
	OpenRow(LANG_FL_tRadacctMonth_uPort,"black");
	printf("<input title='%s' type=text name=uPort value=%u size=16 maxlength=11 "
,LANG_FT_tRadacctMonth_uPort,uPort);
	if(guPermLevel>=20 && uMode)
	{
		printf("></td></tr>\n");
	}
...
//uPortType
	OpenRow(LANG_FL_tRadacctMonth_uPortType,"black");
	printf("<input title='%s' type=text name=uPortType value=%u size=16 maxlength=11 "
,LANG_FT_tRadacctMonth_uPortType,uPortType);
	if(guPermLevel>=20 && uMode)
	{
		printf("></td></tr>\n");
	}
...
//uService
	OpenRow(LANG_FL_tRadacctMonth_uService,"black");
	printf("<input title='%s' type=text name=uService value=%u size=16 maxlength=11 "
,LANG_FT_tRadacctMonth_uService,uService);
	if(guPermLevel>=20 && uMode)
	{
		printf("></td></tr>\n");
	}
...
//uProtocol
	OpenRow(LANG_FL_tRadacctMonth_uProtocol,"black");
	printf("<input title='%s' type=text name=uProtocol value=%u size=16 maxlength=11 "
,LANG_FT_tRadacctMonth_uProtocol,uProtocol);
	if(guPermLevel>=20 && uMode)
	{
		printf("></td></tr>\n");
	}
...
//cIP
	OpenRow(LANG_FL_tRadacctMonth_cIP,"black");
	printf("<input title='%s' type=text name=cIP value=\"%s\" size=40 maxlength=32 "
,LANG_FT_tRadacctMonth_cIP,EncodeDoubleQuotes(cIP));
	if(guPermLevel>=20 && uMode)
	{
		printf("></td></tr>\n");
	}
...
//cLine
	OpenRow(LANG_FL_tRadacctMonth_cLine,"black");
	printf("<input title='%s' type=text name=cLine value=\"%s\" size=40 maxlength=32 "
,LANG_FT_tRadacctMonth_cLine,EncodeDoubleQuotes(cLine));
	if(guPermLevel>=20 && uMode)
	{
		printf("></td></tr>\n");
	}
...
//cCallerID
	OpenRow(LANG_FL_tRadacctMonth_cCallerID,"black");
	printf("<input title='%s' type=text name=cCallerID value=\"%s\" size=40 maxlength=32 "
,LANG_FT_tRadacctMonth_cCallerID,EncodeDoubleQuotes(cCallerID));
	if(guPermLevel>=20 && uMode)
	{
		printf("></td></tr>\n");
	}
...
//uInBytes
	OpenRow(LANG_FL_tRadacctMonth_uInBytes,"black");
	printf("<input title='%s' type=text name=uInBytes value=%lu size=20 maxlength=11 "
,LANG_FT_tRadacctMonth_uInBytes,uInBytes);
	if(guPermLevel>=20 && uMode)
	{
		printf("></td></tr>\n");
	}
...
//uOutBytes
	OpenRow(LANG_FL_tRadacctMonth_uOutBytes,"black");
	printf("<input title='%s' type=text name=uOutBytes value=%u size=16 maxlength=11 "
,LANG_FT_tRadacctMonth_uOutBytes,uOutBytes);
	if(guPermLevel>=20 && uMode)
	{
		printf("></td></tr>\n");
	}
...
//uTermCause
	OpenRow(LANG_FL_tRadacctMonth_uTermCause,"black");
	printf("<input title='%s' type=text name=uTermCause value=%u size=16 maxlength=11 "
,LANG_FT_tRadacctMonth_uTermCause,uTermCause);
	if(guPermLevel>=20 && uMode)
	{
		printf("></td></tr>\n");
	}
...
//uStartTime
	OpenRow(LANG_FL_tRadacctMonth_uStartTime,"black");
	if(uStartTime)
...
//uStopTime
	OpenRow(LANG_FL_tRadacctMonth_uStopTime,"black");
	if(uStopTime)
...
//cInfo
	OpenRow(LANG_FL_tRadacctMonth_cInfo,"black");
	printf("<input title='%s' type=text name=cInfo value=\"%s\" size=40 maxlength=32 "
,LANG_FT_tRadacctMonth_cInfo,EncodeDoubleQuotes(cInfo));
	if(guPermLevel>=20 && uMode)
	{
		printf("></td></tr>\n");
	}
...
void NewtRadacctMonth(unsigned uMode)
{
	register int i=0;
	MYSQL_RES *res;

	sprintf(gcQuery,"SELECT uRadacct FROM tRadacctMonth\
				WHERE uRadacct=%u"
							,uRadacct);
	mysql_query(&gMysql,gcQuery);
	...
	if(i) 
		//tRadacctMonth("<blink>Record already exists");
		tRadacctMonth(LANG_NBR_RECEXISTS);

	//insert query
	Insert_tRadacctMonth();
	if(mysql_errno(&gMysql)) htmlPlainTextError(mysql_error(&gMysql));
...
}//NewtRadacctMonth(unsigned uMode)


void DeletetRadacctMonth(void)
{
#ifdef ISM3FIELDS
	sprintf(gcQuery,"DELETE FROM tRadacctMonth WHERE uRadacct=%u AND ( uOwner=%u OR %u>9 )"
					,uRadacct,guLoginClient,guPermLevel);
#else
	sprintf(gcQuery,"DELETE FROM tRadacctMonth WHERE uRadacct=%u"
					,uRadacct);
#endif
	mysql_query(&gMysql,gcQuery);
	if(mysql_errno(&gMysql)) htmlPlainTextError(mysql_error(&gMysql));

	//tRadacctMonth("Record Deleted");
...
void Insert_tRadacctMonth(void)
{
	...
	//insert query
	sprintf(gcQuery,"INSERT INTO tRadacctMonth SET uRadacct=%u,cLogin='%s',cSessionID='%s',cNAS='%s',uConnectTime=%lu,uPort=%u,uPortType=%u,uService=%u,uProtocol=%u,cIP='%s',cLine='%s',cCallerID='%s',uInBytes=%lu,uOutBytes=%u,uTermCause=%u,uStartTime=%lu,uStopTime=%lu,cInfo='%s'",
			uRadacct
			,TextAreaSave(cLogin)
...
void Update_tRadacctMonth(char *cRowid)
{
	...
	//update query
	sprintf(gcQuery,"UPDATE tRadacctMonth SET uRadacct=%u,cLogin='%s',cSessionID='%s',cNAS='%s',uConnectTime=%lu,uPort=%u,uPortType=%u,uService=%u,uProtocol=%u,cIP='%s',cLine='%s',cCallerID='%s',uInBytes=%lu,uOutBytes=%u,uTermCause=%u,uStartTime=%lu,uStopTime=%lu,cInfo='%s' WHERE _rowid=%s",
			uRadacct
			,TextAreaSave(cLogin)
			,TextAreaSave(cSessionID)
			,TextAreaSave(cNAS)
...
}//void Update_tRadacctMonth(void)


void ModtRadacctMonth(void)
{
	register int i=0;
	MYSQL_RES *res;
	MYSQL_ROW field;
	...
	//Mod select gcQuery
	sprintf(gcQuery,"SELECT uRadacct,uModDate FROM tRadacctMonth WHERE uRadacct=%u"
						,uRadacct);
#else
	sprintf(gcQuery,"SELECT uRadacct FROM tRadacctMonth WHERE uRadacct=%u"
						,uRadacct);
#endif
	...
	//if(i<1) tRadacctMonth("<blink>Record does not exist");
	if(i<1) tRadacctMonth(LANG_NBR_RECNOTEXIST);
	//if(i>1) tRadacctMonth("<blink>Multiple rows!");
	if(i>1) tRadacctMonth(LANG_NBR_MULTRECS);

	field=mysql_fetch_row(res);
#ifdef ISM3FIELDS
	sscanf(field[1],"%u",&uPreModDate);
	if(uPreModDate!=uModDate) tRadacctMonth(LANG_NBR_EXTMOD);
#endif

	Update_tRadacctMonth(field[0]);
	if(mysql_errno(&gMysql)) htmlPlainTextError(mysql_error(&gMysql));
...
}//ModtRadacctMonth(void)


void tRadacctMonthList(void)
{
	MYSQL_RES *res;
	MYSQL_ROW field;

	ExttRadacctMonthListSelect();
	...
	PageMachine("tRadacctMonthList",1,"");//1 is auto header list guMode. Opens table!

	//Filter select drop down
	ExttRadacctMonthListFilter();
