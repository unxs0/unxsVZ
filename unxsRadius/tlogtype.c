...
void NewtLogType(unsigned uMode)
{
	...
	sprintf(gcQuery,"SELECT uLogType FROM tLogType WHERE uLogType=%u",uLogType);
...
{
	sprintf(gcQuery,"DELETE FROM tLogType WHERE uLogType=%u",uLogType);
...
void Insert_tLogType(void)
{
	sprintf(gcQuery,"INSERT INTO tLogType SET uLogType=%u,cLabel='%s',uOwner=%u,uCreatedBy=%u,"
...
void Update_tLogType(char *cRowid)
{
	sprintf(gcQuery,"UPDATE tLogType SET uLogType=%u,cLabel='%s',uModBy=%u,"
