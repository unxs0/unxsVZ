...
FILE 
	main.c
	$Id: main.c 1476 2010-05-24 17:34:16Z Gary $
AUTHOR/LEGAL
	(C) 2010, 2011 Gary Wallis for Unixservice, LLC.
	GPLv2 license applies. See included LICENSE file.
PURPOSE
	unxsvzOrg Interface program file
REQUIRES
...
void htmlLogin(void)
{
	...
	htmlLoginPage("unxsvzOrg","ZLogin.Body");
	htmlFooter("Footer");
...
void htmlFooter(char *cTemplateName)
{
	if(cTemplateName[0])
	{
		...
		if((field=mysql_fetch_row(res)))
		{
			...
			template.cpValue[2]="&copy; 2009-2011 Unixservice, LLC. All Rights Reserved.";
			template.cpName[3]="";
...
void SSLCookieLogin(void)
{
	...
	if(gcCookie[0])
	{
		if((cP=strstr(gcCookie,"unxsvzOrgLogin=")))
		{
			cP+=strlen("unxsvzOrgLogin=");
			if((cP2=strchr(cP,';')))
			{
				*cP2=0;
		...
		if((cP=strstr(gcCookie,"unxsvzOrgPasswd=")))
		{
			cP+=strlen("unxsvzOrgPasswd=");
			if((cP2=strchr(cP,';')))
			{
				*cP2=0;
...
void SetLogin(void)
{
	if(iValidLogin(0))
	{
		printf("Set-Cookie: unxsvzOrgLogin=%s;\n",gcLogin);
		printf("Set-Cookie: unxsvzOrgPasswd=%s;\n",gcPasswd);
		sprintf(gcUser,"%.41s",gcLogin);
		GetPLAndClient(gcUser);
		guSSLCookieLogin=1;
