/*
FILE
	tools/datacenter/subject2sms.c
PURPOSE
	Develop a simple sendmail smrsh compatible
	program for sending SMS messages.
AUTHOR
	Gary Wallis for Unixservice, LLC (C) 2011.
	Contains public domain search code.
*/
...
const char *cBoyermoore(const char *cHaystack,const char *cNeedle);

int main()
{
	char cBuffer[1024]={""};
	char cNeedle[64]={"Subject: "};
	char cSubject[256]={""};
	char *cp;
	char *cp2;
	FILE *fp;
	...
	if((fp=(fopen("/tmp/subject2sms.log","a")))==NULL)
		exit(0);

	while(fgets(cBuffer,1023,stdin)!=NULL)
	{
		fprintf(fp,"%s",cBuffer);

		if((cp=(char *)cBoyermoore(cBuffer,cNeedle)))
		{
			unsigned uLen=strlen(cNeedle);
			if((cp2=strchr(cp+uLen,' ')) || (cp2=strchr(cp+uLen,'\n')) || (cp2=strchr(cp+uLen,'\r')))
				*cp2=0;
			sprintf(cSubject,"%.255s",cp);
			break;
		}
		if(++uLines>100)
			break;
...
//Public domain code from wikipedia
 ...
 */
const char *cBoyermoore(const char *haystack, const char *needle) {
 /*
 * Calc string sizes
 */
