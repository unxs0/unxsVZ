	...
	switch(guMode)
        {
			...
			if(guPermLevel>7)
			{
                ...
                case 3000:
			printf("<u>Authorize: Step 1 Tips</u><br>Depending on the user level you may authorize a contact to access interfaces (like the organization/contact portal.) Or even to use this back-office. In the 'Login' you would enter a login (that can be the same as the tClient.cLabel) for this contact and a password. The most common user permission level is 'Organization Admin' that would allow this contact to login to the idnsOrg.cgi interface and have full control over the companies DNS resource records. The second most common user level is 'Back-Office Root' that will allow the user full access to this back-office mysqlApache2 interface.<p>\n");
			if(guPermLevel>7)
		...
		default:
			...
			if(uAuthorize)
			{
				...
				printf("<a class=darkLink href=mysqlApache2.cgi?gcFunction=tAuthorize&uAuthorize=%u>Go to %s tAuthorize record</a><br>\n",
					uAuthorize
					,cLabel);
			}
...
void ContactsNavList(void)
{
	...
	if(mysql_num_rows(res))
	{
		...
		while((field=mysql_fetch_row(res)))
        	{
			printf("<a class=darkLink href=mysqlApache2.cgi?gcFunction=tClient&uClient=%s&uOnlyASPs=%u"
				,field[0],uOnlyASPs);
			if(cSearch)
			{
				spacetoplus(cSearch);
...
void htmlRecordContext(void)
{
	...
	if(uOwner>1)
		printf("'%s' appears to be an ASP or a reseller owned company or a contact of <a class=darkLink href=mysqlApache2?gcFunction=tClient&uClient=%u>'%s'</a>",cLabel,uOwner,ForeignKey(TCLIENT,"cLabel",uOwner));
...
void tClientNavList(void)
{
	...
	{
		while((field=mysql_fetch_row(res)))
			printf("<a class=darkLink href=mysqlApache2?gcFunction=tClient&uClient=%s&cSearch=%s>%s</a><br>\n",
				field[0]
				,cURLEncode(cSearch)
				,field[1]
				);
