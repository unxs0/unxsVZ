...
FILE
	...
	$Id: tlog.c 1953 2012-05-22 15:03:17Z Colin $
PURPOSE
	...
	if(!cResult[0])
	{
		...
		if(mysql_errno(&gMysql))
			...
			if(strstr(mysql_error(&gMysql)," doesn't exist"))
                	{
				CreatetLog();
				{{cProject}}("New tLog table created");
                	}
			else
			{
...
void DeletetLog(void)
{
	...
	{
#ifdef ISM3FIELDS
		{{cProject}}Log(uLog,"tLog","DelError");
#endif
