/*
FILE
	tJobStatus source code of unxsBlog.cgi
	...
	if(!cResult[0])
	{
		...
		if(mysql_errno(&gMysql))
			...
			if(strstr(mysql_error(&gMysql)," doesn't exist"))
                	{
				CreatetJobStatus();
				unxsBlog("New tJobStatus table created");
                	}
			else
			{
...
void NewtJobStatus(unsigned uMode)
{
	...
	unxsBlogLog(uJobStatus,"tJobStatus","New");
#endif
...
void DeletetJobStatus(void)
{
	...
	{
#ifdef ISM3FIELDS
		unxsBlogLog(uJobStatus,"tJobStatus","Del");
#endif
	...
	{
#ifdef ISM3FIELDS
		unxsBlogLog(uJobStatus,"tJobStatus","DelError");
#endif
...
void ModtJobStatus(void)
{
	...
	unxsBlogLog(uJobStatus,"tJobStatus","Mod");
#endif
	tJobStatus(gcQuery);
