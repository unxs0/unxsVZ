	...
	if(!cResult[0])
	{
		...
		if(mysql_errno(&gMysql))
			...
			if(strstr(mysql_error(&gMysql)," doesn't exist"))
                	{
				CreatetStatus();
				unxsBlog("New tStatus table created");
                	}
			else
			{
...
void NewtStatus(unsigned uMode)
{
	...
	unxsBlogLog(uStatus,"tStatus","New");
#endif
...
void DeletetStatus(void)
{
	...
	{
#ifdef ISM3FIELDS
		unxsBlogLog(uStatus,"tStatus","Del");
#endif
	...
	{
#ifdef ISM3FIELDS
		unxsBlogLog(uStatus,"tStatus","DelError");
#endif
...
void ModtStatus(void)
{
	...
	unxsBlogLog(uStatus,"tStatus","Mod");
#endif
	tStatus(gcQuery);
