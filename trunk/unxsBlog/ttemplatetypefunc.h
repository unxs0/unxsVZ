/*
FILE
	$Id: ttemplatetypefunc.h 2 2006-10-31 13:16:25Z root $
...
void ExttTemplateTypeSelect(void)
{
	...
	sprintf(gcQuery,"SELECT %s FROM tTemplateType WHERE uOwner=%u ORDER BY\
					uTemplateType",
					VAR_LIST_tTemplateType,guLoginClient);
...
void ExttTemplateTypeSelectRow(void)
{
	if(guPermLevel<10)
                sprintf(gcQuery,"SELECT %s FROM tTemplateType,tClient \
                        		...
                        		VAR_LIST_tTemplateType,
					guLoginClient,guLoginClient,uTemplateType);
	else
...
void tTemplateTypeNavList(void)
{
	...
	if(guPermLevel<10)
		sprintf(gcQuery,"SELECT tTemplateType.uTemplateType\
	        ...
	        sprintf(gcQuery,"SELECT uTemplateType,cLabel FROM tTemplateType");
	...
	{	
		...
		{
printf("<a class=darkLink href=unxsBlog.cgi?gcFunction=tTemplateType\
