/*
FILE
	tServer source code of unxsBlog.cgi
	...
	if(!cResult[0])
	{
		...
		if(mysql_errno(&gMysql))
			...
			if(strstr(mysql_error(&gMysql)," doesn't exist"))
                	{
				CreatetServer();
				unxsBlog("New tServer table created");
                	}
			else
			{
...
void NewtServer(unsigned uMode)
{
	...
	unxsBlogLog(uServer,"tServer","New");
#endif
...
void DeletetServer(void)
{
	...
	{
#ifdef ISM3FIELDS
		unxsBlogLog(uServer,"tServer","Del");
#endif
	...
	{
#ifdef ISM3FIELDS
		unxsBlogLog(uServer,"tServer","DelError");
#endif
...
void ModtServer(void)
{
	...
	unxsBlogLog(uServer,"tServer","Mod");
#endif
	tServer(gcQuery);
