...
FILE 
	...
	$Id$
PURPOSE
	Provide wrapper for standalone job queue processor unxsvz
NOTES
	Will keep initialize functions for install in .cgi
	Will move every other command line function here
	including the jobqueue functions.

	First development stage will be to duplicate everything
	here. Then once working fine remove code not required from
	mainfunc.h
AUTHOR/LEGAL
