...
FILE 
	user.c
	$Id: user.c 2812 2015-03-12 14:39:47Z Colin $
AUTHOR/LEGAL
	(C) 2010-2016 Gary Wallis for Unixservice, LLC.
	GPLv2 license applies. See included LICENSE file.
PURPOSE
	unxsVZ interface Portal program file.
	User tab functions.
WORK NOTES
...
void htmlUser(void)
{
	htmlHeader("unxsPortal","UserHeader");
	htmlUserPage("unxsPortal","User.Body");
...
void htmlUserPage(char *cTitle, char *cTemplateName)
{
	if(cTemplateName[0])
	{
		...
		if((field=mysql_fetch_row(res)))
		{
			...
			template.cpValue[9]=LOCALCOPYRIGHT;

			template.cpName[10]="gcInCollapseFive";//Change passwd div
			template.cpValue[10]=gcInCollapseFive;
