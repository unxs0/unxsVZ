/*
FILE
	tAuthorize source code of mysqlRadius2.cgi
	...
	if(!cResult[0])
	{
		...
		if(mysql_errno(&gMysql))
			...
			if(strstr(mysql_error(&gMysql)," doesn't exist"))
                	{
				CreatetAuthorize();
				mysqlRadius2("New tAuthorize table created");
                	}
			else
			{
...
//cPasswd
	...
	printf("<input title='%s' type=text name=cPasswd value=\"%s\" size=40 maxlength=65 "
...
void NewtAuthorize(unsigned uMode)
{
	...
	mysqlRadius2Log(uAuthorize,TAUTHORIZE,"New");
...
void DeletetAuthorize(void)
{
	...
	{
		mysqlRadius2Log(uAuthorize,TAUTHORIZE,"Del");
		tAuthorize(LANG_NBR_RECDELETED);
	}
	else
	{
		mysqlRadius2Log(uAuthorize,TAUTHORIZE,"DelError");
...
void ModtAuthorize(void)
{
	...
	mysqlRadius2Log(uAuthorize,TAUTHORIZE,"Mod");
	tAuthorize(gcQuery);
...
{
	sprintf(gcQuery,"CREATE TABLE IF NOT EXISTS tAuthorize ( uAuthorize INT UNSIGNED PRIMARY KEY AUTO_INCREMENT, cLabel VARCHAR(32) NOT NULL DEFAULT '', UNIQUE (cLabel,uOwner), uOwner INT UNSIGNED NOT NULL DEFAULT 0,index (uOwner), uCreatedBy INT UNSIGNED NOT NULL DEFAULT 0, uCreatedDate INT UNSIGNED NOT NULL DEFAULT 0, uModBy INT UNSIGNED NOT NULL DEFAULT 0, uModDate INT UNSIGNED NOT NULL DEFAULT 0, cIpMask VARCHAR(20) NOT NULL DEFAULT '', uPerm INT UNSIGNED NOT NULL DEFAULT 0, uCertClient INT UNSIGNED NOT NULL DEFAULT 0, cPasswd VARCHAR(64) NOT NULL DEFAULT '', cClrPasswd VARCHAR(32) NOT NULL DEFAULT '' )");
	macro_mySQLQueryHTMLError;
