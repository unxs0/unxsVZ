/*
FILE
	tUserConfig source code of mysqlMail2.cgi
	Built by mysqlRAD2.cgi (C) Gary Wallis and Hugo Urquiza 2001-2009
	$Id: tuserconfig.c 2316 2008-12-19 19:40:15Z hus-admin $
PURPOSE
	...
	if(!cResult[0])
	{
		...
		if(mysql_errno(&gMysql))
			...
			if(strstr(mysql_error(&gMysql)," doesn't exist"))
                	{
				CreatetUserConfig();
				mysqlMail2("New tUserConfig table created");
                	}
			else
			{
...
void NewtUserConfig(unsigned uMode)
{
	...
	mysqlMail2Log(uUserConfig,"tUserConfig","New");
#endif
...
void DeletetUserConfig(void)
{
...
#else
	sprintf(gcQuery,"DELETE FROM tUserConfig WHERE uUserConfig=%u"
					,uUserConfig);
#endif
	mysql_query(&gMysql,gcQuery);
	if(mysql_errno(&gMysql)) htmlPlainTextError(mysql_error(&gMysql));
	...
	{
#ifdef ISM3FIELDS
		mysqlMail2Log(uUserConfig,"tUserConfig","Del");
#endif
	...
	{
#ifdef ISM3FIELDS
		mysqlMail2Log(uUserConfig,"tUserConfig","DelError");
#endif
...
void ModtUserConfig(void)
{
	...
	mysqlMail2Log(uUserConfig,"tUserConfig","Mod");
#endif
	tUserConfig(gcQuery);
