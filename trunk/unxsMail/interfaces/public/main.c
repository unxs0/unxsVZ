...
void htmlPlainTextError(const char *cError)
{
	...
	printf("Please report this iMail2 fatal error ASAP:\n%s\n",cError);
...
void ConnectDb(void)
{
        ...
        {
		htmlHeader("index.cgi Error","Header");
...
sprintf(gcQuery,"SELECT uJob FROM tJob WHERE cLabel='%.99s' AND cServer='%.64s' AND cJobName='%.32s' AND uJobTarget=%u AND uJobTargetUser=%u AND cJobData='%.254s' AND uJobStatus=1 AND uOwner=%u AND uCreatedBy=%u",cLabel,cServer,cJobName,uJobTarget,uJobTargetUser,cJobData,uOwner,uCreatedBy);
		mysql_query(&gMysql,gcQuery);
		if(mysql_errno(&gMysql)) htmlPlainTextError(gcQuery);
		...
		{
sprintf(gcQuery,"INSERT INTO tJob SET cLabel='%.99s',cServer='%.64s',cJobName='%.32s',uJobTarget=%u,uJobTargetUser=%u,cJobData='%.254s',uJobDate=UNIX_TIMESTAMP(NOW()),uJobStatus=1,uOwner=%u,uCreatedBy=%u,uCreatedDate=UNIX_TIMESTAMP(NOW())",cLabel,cServer,cJobName,uJobTarget,uJobTargetUser,cJobData,uOwner,uCreatedBy);
			mysql_query(&gMysql,gcQuery);
			if(mysql_errno(&gMysql)) htmlPlainTextError(gcQuery);
		}
		else
