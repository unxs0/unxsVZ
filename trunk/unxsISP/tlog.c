...
FILE
	...
	$Id: tlog.c 1335 2007-07-23 23:15:44Z Gary $
PURPOSE
	...
	if(!cResult[0])
	{
		...
		if(mysql_errno(&gMysql))
			...
			if(strstr(mysql_error(&gMysql)," doesn't exist"))
                	{
				CreatetLog();
				mysqlISP2("New tLog table created");
                	}
			else
			{
...
//uLog
	...
	printf("<input title='%s' type=text name=uLog value=%u size=16 maxlength=10 ",LANG_FT_tLog_uLog,uLog);
...
//cMessage
	...
	printf("<textarea title='%s' cols=80 wrap=hard rows=4 name=cMessage ",
...
void DeletetLog(void)
{
	...
	{
#ifdef ISM3FIELDS
		mysqlISP2Log(uLog,"tLog","DelError");
#endif
