/*
FILE
	tPaid source code of mysqlISP2.cgi
	...
	if(!cResult[0])
	{
		...
		if(mysql_errno(&gMysql))
			...
			if(strstr(mysql_error(&gMysql)," doesn't exist"))
                	{
				CreatetPaid();
				mysqlISP2("New tPaid table created");
                	}
			else
			{
...
void DeletetPaid(void)
{
	...
	{
#ifdef ISM3FIELDS
		mysqlISP2Log(uPaid,"tPaid","DelError");
#endif
