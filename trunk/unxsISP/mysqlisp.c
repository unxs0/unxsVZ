...
void ProcessJobQueue(unsigned const uDebug, char const *cServer)
{
	...
	sprintf(gcQuery,"SELECT uJob,cJobName FROM tJob WHERE (cServer='Any' OR cServer='%s') AND uJobStatus=%u AND uJobDate<%lu ORDER BY uJob LIMIT 10",cServer,mysqlISP_Waiting,luClock);
        ...
        while((field=mysql_fetch_row(res)))
	{
		...
		if(!strncmp(field[1],"mysqlISP.tInvoice.Email",23))
			EmailInvoicesFromJobQueue(uJob);
		else if(!strncmp(field[1],"mysqlISP.tInvoice.Print",23))
...
void PrintInvoicesFromJobQueue(unsigned const uJob)
{
	...
	char cInvoiceBaseDir[256]={"/home/openisp/mysqlISP/invoices"};
	...
	sprintf(gcQuery,"SELECT tInvoice.uInvoice,tClient.uClient,tInvoice.cFirstName,tInvoice.cLastName,tClient.cLabel,tInvoice.cEmail,tInvoice.cAddr1,tInvoice.cAddr2,tInvoice.cCity,tInvoice.cState,tInvoice.cZip,tInvoice.cCountry,tInvoice.mTotal,tInvoice.mSH,tInvoice.mTax,tInvoice.cComments FROM tInvoice,tClient WHERE tInvoice.uClient=tClient.uClient AND tInvoice.uQueueStatus=%u ORDER BY uInvoice",mysqlISP_PrintQueuedInvoice);
        mysql_query(&gMysql,gcQuery);
        if(mysql_errno(&gMysql))
	{
		fprintf(stderr,"%s\n",mysql_error(&gMysql));
...
*/
		...
		fprintf(fp,"\n");
		fprintf(fp,"\n\n(Invoice generated by mysqlISP: %s)\n",RELEASE);
		fclose(fp);
	...
	UpdateJobStatus(uJob,mysqlISP_Deployed,gcQuery);
...
void EmailInvoicesFromJobQueue(unsigned const uJob)
{
	...
	sprintf(gcQuery,"SELECT tInvoice.uInvoice,tClient.uClient,tInvoice.cFirstName,tInvoice.cLastName,tClient.cLabel,tInvoice.cEmail,tInvoice.cAddr1,tInvoice.cAddr2,tInvoice.cCity,tInvoice.cState,tInvoice.cZip,tInvoice.cCountry,tInvoice.mTotal,tInvoice.mSH,tInvoice.mTax,tInvoice.cComments,tInvoice.uInvoiceStatus FROM tInvoice,tClient WHERE tInvoice.uClient=tClient.uClient AND tInvoice.uQueueStatus=%u ORDER BY uInvoice",mysqlISP_MailQueuedInvoice);
        mysql_query(&gMysql,gcQuery);
        if(mysql_errno(&gMysql))
	{
		fprintf(stderr,"%s\n",mysql_error(&gMysql));
