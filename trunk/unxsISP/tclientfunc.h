	...
	if(!strcmp(gcFunction,"tClientTools"))
	{
		if(!strcmp(gcCommand,LANG_NB_NEW))
			...
			if(uAllowDel(uOwner,uCreatedBy))
			{
				...
				sprintf(gcQuery,"SELECT uConfig FROM tConfig WHERE uOwner=%u",uClient);
				mysql_query(&gMysql,gcQuery);
        			if(mysql_errno(&gMysql))
                			htmlPlainTextError(mysql_error(&gMysql));
        			res=mysql_store_result(&gMysql);
				if(mysql_num_rows(res))
					tClient("<blink>Error</blink>: Can't delete client with tConfig entries");
				mysql_free_result(res);
				if(IsAuthUser(cLabel,uOwner,uClient))
				{
	...
	switch(guMode)
        {
			...
			if(guPermLevel>7)
			{
                ...
                case 3000:
			printf("<u>Authorize: Step 1 Tips</u><br>Depending on the user level you may authorize a contact to access interfaces (like the organization/contact portal.) Or even to use this back-office. In the 'Login' you would enter a login (that can be the same as the tClient.cLabel) for this contact and a password. The most common user permission level is 'Organization Admin' that would allow this contact to login to the idnsOrg.cgi interface and have full control over the companies DNS resource records. The second most common user level is 'Back-Office Root' that will allow the user full access to this back-office mysqlISP2.cgi interface.<p>\n");
			if(guPermLevel>7)
		...
		default:
			...
			uAuthorize=IsAuthUser(cLabel,uOwner,uClient);

			if(uClient && guPermLevel>9 && uClient!=guLoginClient 
				&& !uAuthorize &&guMode!=5 && uOwner!=1)
			{
			...
			if(uAuthorize)
			{
				...
				printf("<a class=darkLink href=mysqlISP2.cgi?gcFunction=tAuthorize&uAuthorize=%u>Go to %s tAuthorize record</a><br>\n",
					uAuthorize
					,cLabel);
			}
...
void ExttClientListSelect(void)
{
	...
	//Changes here must be reflected below in ExttClientListFilter()
		...
		sprintf(cCat,"uClient=%u ORDER BY uClient",uClient);
		strcat(gcQuery,cCat);
        }
	...
	else if(!strcmp(gcFilter,"cFirstName"))
        {
		sprintf(cCat,"cFirstName LIKE '%s' ORDER BY cFirstName",gcCommand);
		strcat(gcQuery,cCat);
	}
	else if(!strcmp(gcFilter,"cLastName"))
        {
		sprintf(cCat,"cLastName LIKE '%s' ORDER BY cLastName",gcCommand);
		strcat(gcQuery,cCat);
	}
	else if(!strcmp(gcFilter,"cEmail"))
        {
		sprintf(cCat,"cEmail LIKE '%s' ORDER BY cEmail",gcCommand);
		strcat(gcQuery,cCat);
	}
	else if(!strcmp(gcFilter,"cCity"))
        {
		sprintf(cCat,"cCity LIKE '%s' ORDER BY cCity",gcCommand);
		strcat(gcQuery,cCat);
	}
	else if(!strcmp(gcFilter,"cState"))
        {
		sprintf(cCat,"cState LIKE '%s' ORDER BY cState",gcCommand);
		strcat(gcQuery,cCat);
	}
	else if(!strcmp(gcFilter,"cCountry"))
        {
		sprintf(cCat,"cCountry LIKE '%s' ORDER BY cCountry",gcCommand);
		strcat(gcQuery,cCat);
	}
        else if(1)
        {
		...
		strcat(gcQuery," uClient>0 ORDER BY uClient");
        }
...
void ExttClientListFilter(void)
{
        ...
        if(strcmp(gcFilter,"cFirstName"))
                printf("<option>cFirstName</option>");
        else
                printf("<option selected>cFirstName</option>");
        if(strcmp(gcFilter,"cLastName"))
                printf("<option>cLastName</option>");
        else
                printf("<option selected>cLastName</option>");
        if(strcmp(gcFilter,"cEmail"))
                printf("<option>cEmail</option>");
        else
                printf("<option selected>cEmail</option>");
        if(strcmp(gcFilter,"cCity"))
                printf("<option>cCity</option>");
        else
                printf("<option selected>cCity</option>");
        if(strcmp(gcFilter,"cState"))
                printf("<option>cState</option>");
        else
                printf("<option selected>cState</option>");
        if(strcmp(gcFilter,"cCountry"))
                printf("<option>cCountry</option>");
        else
                printf("<option selected>cCountry</option>");
...
void ContactsNavList(void)
{
	...
	if(mysql_num_rows(res))
	{
		...
		while((field=mysql_fetch_row(res)))
        	{
			printf("<a class=darkLink href=mysqlISP2.cgi?gcFunction=tClient&uClient=%s&uOnlyASPs=%u"
				,field[0],uOnlyASPs);
			if(cSearch)
			{
				spacetoplus(cSearch);
...
void htmlRecordContext(void)
{
	...
	if(uOwner>1)
		printf("'%s' appears to be an ASP or a reseller owned company or a contact of <a class=darkLink href=mysqlISP2.cgi?gcFunction=tClient&uClient=%u>'%s'</a>",cLabel,uOwner,ForeignKey(TCLIENT,"cLabel",uOwner));
...
void tClientNavList(void)
{
	...
	{
		while((field=mysql_fetch_row(res)))
			printf("<a class=darkLink href=mysqlISP2.cgi?gcFunction=tClient&uClient=%s&cSearch=%s>%s</a><br>\n",
				field[0]
				,cURLEncode(cSearch)
				,field[1]
				);
