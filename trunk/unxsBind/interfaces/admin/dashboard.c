...
char *cGetSystemHealth(void)
{
	...
	sprintf(gcQuery,"SELECT COUNT(uLog) FROM tLog WHERE uLogType=5 AND ((uCreatedDate >= UNIX_TIMESTAMP(NOW())-86400) OR (uModDate>= UNIX_TIMESTAMP(NOW())-86400))");
	mysql_query(&gMysql,gcQuery);
	if(mysql_errno(&gMysql))
		htmlPlainTextError(mysql_error(&gMysql));
	res=mysql_store_result(&gMysql);
	...
	sprintf(gcQuery,"SELECT COUNT(uLog) FROM tLog WHERE uLogType=4 AND cMessage LIKE '%%dnsMonitor%%' AND ((uCreatedDate >= UNIX_TIMESTAMP(NOW())-3600) OR (uModDate>= UNIX_TIMESTAMP(NOW())-3600))");
	mysql_query(&gMysql,gcQuery);
	if(mysql_errno(&gMysql))
		htmlPlainTextError(mysql_error(&gMysql));
	res=mysql_store_result(&gMysql);
