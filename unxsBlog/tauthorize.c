/*
FILE
	tAuthorize source code of unxsBlog.cgi
	...
	if(!cResult[0])
	{
		...
		if(mysql_errno(&gMysql))
			...
			if(strstr(mysql_error(&gMysql)," doesn't exist"))
                	{
				CreatetAuthorize();
				unxsBlog("New tAuthorize table created");
                	}
			else
			{
...
void NewtAuthorize(unsigned uMode)
{
	...
	unxsBlogLog(uAuthorize,"tAuthorize","New");
#endif
...
void DeletetAuthorize(void)
{
	...
	{
#ifdef ISM3FIELDS
		unxsBlogLog(uAuthorize,"tAuthorize","Del");
#endif
	...
	{
#ifdef ISM3FIELDS
		unxsBlogLog(uAuthorize,"tAuthorize","DelError");
#endif
...
void ModtAuthorize(void)
{
	...
	unxsBlogLog(uAuthorize,"tAuthorize","Mod");
#endif
	tAuthorize(gcQuery);
