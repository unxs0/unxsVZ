	...
	if(!cResult[0])
	{
		...
		if(mysql_errno(&gMysql))
			...
			if(strstr(mysql_error(&gMysql)," doesn't exist"))
                	{
				CreatetGlossary();
				unxsBlog("New tGlossary table created");
                	}
			else
			{
...
void NewtGlossary(unsigned uMode)
{
	...
	unxsBlogLog(uGlossary,"tGlossary","New");
#endif
...
void DeletetGlossary(void)
{
	...
	{
#ifdef ISM3FIELDS
		unxsBlogLog(uGlossary,"tGlossary","Del");
#endif
	...
	{
#ifdef ISM3FIELDS
		unxsBlogLog(uGlossary,"tGlossary","DelError");
#endif
...
void ModtGlossary(void)
{
	...
	unxsBlogLog(uGlossary,"tGlossary","Mod");
#endif
	tGlossary(gcQuery);
