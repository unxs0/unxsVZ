	...
	if(!cResult[0])
	{
		...
		if(mysql_errno(&gMysql))
			...
			if(strstr(mysql_error(&gMysql)," doesn't exist"))
                	{
				CreatetLogType();
				unxsBlog("New tLogType table created");
                	}
			else
			{
...
void NewtLogType(unsigned uMode)
{
	...
	unxsBlogLog(uLogType,"tLogType","New");
#endif
...
void DeletetLogType(void)
{
	...
	{
#ifdef ISM3FIELDS
		unxsBlogLog(uLogType,"tLogType","Del");
#endif
	...
	{
#ifdef ISM3FIELDS
		unxsBlogLog(uLogType,"tLogType","DelError");
#endif
...
void ModtLogType(void)
{
	...
	unxsBlogLog(uLogType,"tLogType","Mod");
#endif
	tLogType(gcQuery);
