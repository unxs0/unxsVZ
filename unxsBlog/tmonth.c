	...
	if(!cResult[0])
	{
		...
		if(mysql_errno(&gMysql))
			...
			if(strstr(mysql_error(&gMysql)," doesn't exist"))
                	{
				CreatetMonth();
				unxsBlog("New tMonth table created");
                	}
			else
			{
...
void DeletetMonth(void)
{
	...
	{
#ifdef ISM3FIELDS
		unxsBlogLog(uMonth,"tMonth","DelError");
#endif
