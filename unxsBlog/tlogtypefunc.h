...
void ExttLogTypeSelect(void)
{
	...
	sprintf(gcQuery,"SELECT %s FROM tLogType WHERE uOwner=%u ORDER BY\
					uLogType",
					VAR_LIST_tLogType,guLoginClient);
...
void ExttLogTypeSelectRow(void)
{
	if(guPermLevel<10)
                sprintf(gcQuery,"SELECT %s FROM tLogType,tClient \
                        		...
                        		VAR_LIST_tLogType,
					guLoginClient,guLoginClient,uLogType);
	else
...
void tLogTypeNavList(void)
{
	...
	if(guPermLevel<10)
		sprintf(gcQuery,"SELECT tLogType.uLogType\
	        ...
	        sprintf(gcQuery,"SELECT uLogType,cLabel FROM tLogType");
	...
	{	
		...
		{
printf("<a class=darkLink href=unxsBlog.cgi?gcFunction=tLogType\
&uLogType=%s>%s</a><br>\n",field[0],field[1]);
	        }
	}
