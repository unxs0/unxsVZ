/*
FILE
	tClient source code of unxsBlog.cgi
	Built by mysqlRAD2.cgi (C) Gary Wallis 2001-2007
	$Id: tclient.c.template 1292 2007-07-09 23:31:18Z Gary $
PURPOSE
	...
	if(!cResult[0])
	{
		...
		if(mysql_errno(&gMysql))
			...
			if(strstr(mysql_error(&gMysql)," doesn't exist"))
                	{
				CreatetClient();
				unxsBlog("New tClient table created");
                	}
			else
			{
...
void NewtClient(unsigned uMode)
{
	...
	unxsBlogLog(uClient,"tClient","New");
#endif
...
void DeletetClient(void)
{
	...
	{
#ifdef ISM3FIELDS
		unxsBlogLog(uClient,"tClient","Del");
#endif
	...
	{
#ifdef ISM3FIELDS
		unxsBlogLog(uClient,"tClient","DelError");
#endif
...
void ModtClient(void)
{
	...
	unxsBlogLog(uClient,"tClient","Mod");
#endif
	tClient(gcQuery);
