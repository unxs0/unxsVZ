/*
FILE
	tTemplateSet source code of unxsBlog.cgi
	...
	if(!cResult[0])
	{
		...
		if(mysql_errno(&gMysql))
			...
			if(strstr(mysql_error(&gMysql)," doesn't exist"))
                	{
				CreatetTemplateSet();
				unxsBlog("New tTemplateSet table created");
                	}
			else
			{
...
void DeletetTemplateSet(void)
{
	...
	{
#ifdef ISM3FIELDS
		unxsBlogLog(uTemplateSet,"tTemplateSet","Del");
#endif
	...
	{
#ifdef ISM3FIELDS
		unxsBlogLog(uTemplateSet,"tTemplateSet","DelError");
#endif
...
void ModtTemplateSet(void)
{
	...
	unxsBlogLog(uTemplateSet,"tTemplateSet","Mod");
#endif
	tTemplateSet(gcQuery);
