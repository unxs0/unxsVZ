	...
	if(!strcmp(gcFunction,"tClientTools"))
	{
		if(!strcmp(gcCommand,LANG_NB_NEW))
				...
				|| (guPermLevel>7 && guReseller==guLoginClient) )
			{
				...
				sprintf(gcQuery,"DELETE FROM tAuthorize WHERE (cLabel='%s' OR uCertClient=%u) AND (uOwner=%u OR uOwner=%u)",cLabel,uClient,uClient,guLoginClient);
				mysql_query(&gMysql,gcQuery);
        			if(mysql_errno(&gMysql))
                			tClient(mysql_error(&gMysql));
                        	guMode=5;
			...
			if( uClient && (guPermLevel>9 || 
					(guPermLevel>7 && uOwner==guLoginClient)))
			{
				guMode=3000;
			...
			//and only for new Globix customer/contact model.
                        ProcesstClientVars(entries,x);
			if( uClient && guPermLevel>9 )
			{
				time_t clock;
...
sprintf(gcQuery,"INSERT INTO tAuthorize SET cLabel='%s',uPerm=%u,uCertClient=%u,cPasswd='%s',uOwner=%u,uCreatedBy=%u,uCreatedDate=UNIX_TIMESTAMP(NOW()),cIPMask='0.0.0.0',cClrPasswd='%s'",
	...
	switch(guMode)
        {
			...
			if(guPermLevel>7)
			{
                ...
                case 3000:
			printf("<u>Authorize: Step 1 Tips</u><br>Depending on the user level you may authorize a contact to access interfaces (like the organization/contact portal.) Or even to use this back-office. In the 'Login' you would enter a login (that can be the same as the tClient.cLabel) for this contact and a password. The most common user permission level is 'Organization Admin' that would allow this contact to login to the idnsOrg.cgi interface and have full control over the companies DNS resource records. The second most common user level is 'Back-Office Root' that will allow the user full access to this back-office unxsBlog.cgi interface.<p>\n");
			if(guPermLevel>7)
		...
		default:
			if(guPermLevel>9)
			{
				...
				printf("Here you can create new companies or add new contacts to existing companies. Once a new contact is made you can authorize her to use the interfaces or even this backoffice web interface.");
				...
				printf("Only Orgs/Companies <input title='Limit search to Root owned tClient records that in this model are companies' type=checkbox name=uOnlyOrgs ");
				if(uOnlyOrgs)
					printf("checked><br>");
				else
					printf("><br>");
...
void ExttClientSelect(void)
{
	...
	if(guPermLevel>=9)
	{
		if(cSearch[0] && uOnlyOrgs )
			sprintf(gcQuery,"SELECT %s FROM tClient WHERE cLabel LIKE '%s%%' AND uOwner=1 AND uClient!=1 ORDER BY cLabel",VAR_LIST_tClient,cSearch);//Depends on Root client being uClient=1
	...
	{
	sprintf(gcQuery,"SELECT %s FROM tClient WHERE uOwner=%u OR uClient=%u ORDER BY uClient",VAR_LIST_tClient,guLoginClient,guLoginClient);
	}
...
void ContactsNavList(void)
{
	...
	if(uOwner!=1)
	{
		...
		if(mysql_num_rows(res))
		{
        		printf("<p><u>tAuthorize LoginNavList</u><br>\n");
			while((field=mysql_fetch_row(res)))
				printf("<a class=darkLink href=unxsBlog.cgi?gcFunction=tAuthorize&uAuthorize=%s>%s<a><br>",field[1],field[0]);
		}
        	mysql_free_result(res);
		printf("\n");
	}
	...
	if(mysql_num_rows(res))
	{
		...
		while((field=mysql_fetch_row(res)))
        	{
			printf("<a class=darkLink href=unxsBlog.cgi?gcFunction=tClient&uClient=%s&uOnlyOrgs=%u",field[0],uOnlyOrgs);
			if(cSearch)
			{
				spacetoplus(cSearch);
...
void htmlRecordContext(void)
{
	...
	if(uOwner>1)
		printf("'%s' appears to be a contact of <a class=darkLink href=unxsBlog.cgi?gcFunction=tClient&uClient=%u>'%s'</a>",cLabel,uOwner,ForeignKey("tClient","cLabel",uOwner));
