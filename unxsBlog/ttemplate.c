/*
FILE
	tTemplate source code of unxsBlog.cgi
	Built by mysqlRAD2.cgi (C) Gary Wallis 2001-2007
	$Id: ttemplate.c 42 2006-11-08 16:26:27Z root $
PURPOSE
	...
	if(!cResult[0])
	{
		...
		if(mysql_errno(&gMysql))
			...
			if(strstr(mysql_error(&gMysql)," doesn't exist"))
                	{
				CreatetTemplate();
				unxsBlog("New tTemplate table created");
                	}
			else
			{
...
void NewtTemplate(unsigned uMode)
{
	...
	unxsBlogLog(uTemplate,"tTemplate","New");
#endif
...
void DeletetTemplate(void)
{
	...
	{
#ifdef ISM3FIELDS
		unxsBlogLog(uTemplate,"tTemplate","Del");
#endif
	...
	{
#ifdef ISM3FIELDS
		unxsBlogLog(uTemplate,"tTemplate","DelError");
#endif
...
void ModtTemplate(void)
{
	...
	sprintf(gcQuery,"SELECT tTemplate.uTemplate,\
				...
				WHERE tTemplate.uTemplate=%u\
				AND tTemplate.uOwner=tClient.uClient\
	...
	unxsBlogLog(uTemplate,"tTemplate","Mod");
#endif
	tTemplate(gcQuery);
...
void tTemplateList(void)
{
	...
	for(guN=0;guN<(guEnd-guStart+1);guN++)
	{
		...
		if(luTime8)
			...
			sprintf(cBuf8,"---");
		long unsigned luTime10=strtoul(field[10],NULL,10);
		char cBuf10[32];
		if(luTime10)
