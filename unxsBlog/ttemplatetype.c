/*
FILE
	tTemplateType source code of unxsBlog.cgi
	...
	if(!cResult[0])
	{
		...
		if(mysql_errno(&gMysql))
			...
			if(strstr(mysql_error(&gMysql)," doesn't exist"))
                	{
				CreatetTemplateType();
				unxsBlog("New tTemplateType table created");
                	}
			else
			{
...
void NewtTemplateType(unsigned uMode)
{
	...
	unxsBlogLog(uTemplateType,"tTemplateType","New");
#endif
...
void DeletetTemplateType(void)
{
	...
	{
#ifdef ISM3FIELDS
		unxsBlogLog(uTemplateType,"tTemplateType","Del");
#endif
	...
	{
#ifdef ISM3FIELDS
		unxsBlogLog(uTemplateType,"tTemplateType","DelError");
#endif
...
void ModtTemplateType(void)
{
	...
	unxsBlogLog(uTemplateType,"tTemplateType","Mod");
#endif
	tTemplateType(gcQuery);
	...
	for(guN=0;guN<(guEnd-guStart+1);guN++)
	{
		...
		if(!field)
		{
			...
			if(guN % 2)
				...
				printf("<tr>");
		long unsigned luTime4=strtoul(field[4],NULL,10);
		char cBuf4[32];
		if(luTime4)
