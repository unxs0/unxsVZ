/*
FILE
	tTemplateType source code of unxsVZ.cgi
	Built by mysqlRAD2.cgi (C) Gary Wallis 2001-2007
PURPOSE
	Template type codes for template clasification per interface or similar
	use.
AUTHOR/LEGAL
	...
	if(!cResult[0])
	{
		...
		if(mysql_errno(&gMysql))
			...
			if(strstr(mysql_error(&gMysql)," doesn't exist"))
                	{
				CreatetTemplateType();
				unxsVZ("New tTemplateType table created");
                	}
			else
			{
...
void NewtTemplateType(unsigned uMode)
{
	...
	unxsVZLog(uTemplateType,"tTemplateType","New");
#endif
...
void DeletetTemplateType(void)
{
	...
	{
#ifdef ISM3FIELDS
		unxsVZLog(uTemplateType,"tTemplateType","Del");
#endif
	...
	{
#ifdef ISM3FIELDS
		unxsVZLog(uTemplateType,"tTemplateType","DelError");
#endif
...
void ModtTemplateType(void)
{
	...
	sprintf(gcQuery,"SELECT tTemplateType.uTemplateType,\
				tTemplateType.uModDate\
				FROM tTemplateType,tClient\
				WHERE tTemplateType.uTemplateType=%u\
				AND tTemplateType.uOwner=tClient.uClient\
				AND (tClient.uOwner=%u OR tClient.uClient=%u)"
			,uTemplateType,guLoginClient,guLoginClient);
	else
	sprintf(gcQuery,"SELECT uTemplateType,uModDate FROM tTemplateType\
				WHERE uTemplateType=%u"
						,uTemplateType);
#else
	sprintf(gcQuery,"SELECT uTemplateType FROM tTemplateType\
	...
	unxsVZLog(uTemplateType,"tTemplateType","Mod");
#endif
	tTemplateType(gcQuery);
...
void tTemplateTypeList(void)
{
	...
	for(guN=0;guN<(guEnd-guStart+1);guN++)
	{
		...
		if(luTime6)
			...
			sprintf(cBuf6,"---");
		printf("<td><a class=darkLink href=unxsVZ.cgi?gcFunction=tTemplateType&uTemplateType=%s>%s</a>"
			,field[0]
			,field[0]);
...
{
	sprintf(gcQuery,"CREATE TABLE IF NOT EXISTS tTemplateType ("
		" uTemplateType INT UNSIGNED PRIMARY KEY AUTO_INCREMENT,"
