/*
FILE
	$Id: tlogfunc.h 2317 2008-12-19 20:39:19Z hus-admin $
...
void ExttLogButtons(void)
{
	...
	printf("This table holds the non-archived logged and MD5 signed operations that have taken place in the system. "
		...
		"Current mysqlMail2 version does not save delete (Del) operation data.<p><a href=iDNS.cgi?gcFunction=tLogMonth>tLogMonth</a> "
	...
	if(uLog && cLabel[0] && guPermLevel>10 && uTablePK[0] && cTableName[0])
	{
		...
		else if(!strcmp(cTableName,"tResource"))
		{
			...
			if( uTPK && uZone && uRRType)
			{
				...
				if(!strcmp(cLabel,"Del"))
					printf("<a title='Jump to tDeletedResource entry' href=mysqlMail2.cgi?gcFunction=tDeletedResource&uDeletedResource=%u>"
						"tDeletedResource</a><blockquote>\n",uTPK);
				else
					printf("<a title='Jump to tResource entry' href=mysqlMail2.cgi?gcFunction=tResource&uResource=%u>tResource</a>"
						"<blockquote>\n",uTPK);
			...
			if(uLoginClient)
			{
				printf("Contact=<a title='Jump to tClient entry' href=mysqlMail2.cgi?gcFunction=tClient&uClient=%u>%s</a><br></blockquote>\n",
					uLoginClient,
