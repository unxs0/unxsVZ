...
void tServerContextInfo(void)
{
	...
	printf("<u>Groups</u><br>\n");
	sprintf(gcQuery,"SELECT tServerGroup.uServerGroup,tServerGroup.cLabel FROM tServer,tServerGroup,tServerGroupGlue WHERE tServer.uServer=tServerGroupGlue.uServer AND tServerGroupGlue.uServerGroup=tServerGroup.uServerGroup AND tServer.uServer=%u",uServer);
        mysql_query(&gMysql,gcQuery);
        if(mysql_errno(&gMysql))
        {
                printf("%s",mysql_error(&gMysql));
