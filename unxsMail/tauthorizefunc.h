	...
	switch(guMode)
		...
		default:
			...
			if(uCertClient>1 && uOwner>1)
				printf("This login appears to belong to a <a class=darkLink href=unxsMail.cgi?gcFunction=tClient&uClient=%u>'%s'</a> company contact '<a class=darkLink href=unxsVZ.cgi?gcFunction=tClient&uClient=%u>%s</a>'.<br>The uPerm corresponds to permission level '%s'.",uOwner,ForeignKey("tClient","cLabel",uOwner),uCertClient,ForeignKey("tClient","cLabel",uCertClient),cUserLevel(uPerm));
			else if(uOwner>1)
				printf("This login appears to belong to a '<a class=darkLink href=unxsMail.cgi?gcFunction=tClient&uClient=%u>%s</a>' company contact, but that has been root aliased to usually run the back-office with complete permissions. <br>The uPerm corresponds to permission level '%s'.",uOwner,ForeignKey("tClient","cLabel",uOwner),cUserLevel(uPerm));
			printf("<p>\n");
			tAuthorizeNavList();
	}
...
void tAuthorizeNavList(void)
{
	...
	if(mysql_num_rows(res))
	{
			...
			printf("<a class=darkLink href=unxsMail.cgi?gcFunction=tAuthorize&uAuthorize=%s>"
