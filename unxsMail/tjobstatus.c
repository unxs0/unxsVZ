/*
FILE
	tJobStatus source code of mysqlMail2.cgi
	Built by mysqlRAD2.cgi (C) Gary Wallis and Hugo Urquiza 2001-2009
	$Id: tjobstatus.c 2316 2008-12-19 19:40:15Z hus-admin $
PURPOSE
	...
	if(!cResult[0])
	{
		...
		if(mysql_errno(&gMysql))
			...
			if(strstr(mysql_error(&gMysql)," doesn't exist"))
                	{
				CreatetJobStatus();
				mysqlMail2("New tJobStatus table created");
                	}
			else
			{
...
void NewtJobStatus(unsigned uMode)
{
	...
	mysqlMail2Log(uJobStatus,"tJobStatus","New");
#endif
...
void DeletetJobStatus(void)
{
	...
	{
#ifdef ISM3FIELDS
		mysqlMail2Log(uJobStatus,"tJobStatus","Del");
#endif
	...
	{
#ifdef ISM3FIELDS
		mysqlMail2Log(uJobStatus,"tJobStatus","DelError");
#endif
...
void ModtJobStatus(void)
{
	...
	//Mod select gcQuery
	sprintf(gcQuery,"SELECT uJobStatus,uModDate FROM tJobStatus WHERE uJobStatus=%u"
						,uJobStatus);
#else
	sprintf(gcQuery,"SELECT uJobStatus FROM tJobStatus\
				WHERE uJobStatus=%u"
	...
	mysqlMail2Log(uJobStatus,"tJobStatus","Mod");
#endif
	tJobStatus(gcQuery);
