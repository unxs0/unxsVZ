...
void ExttJobCommands(pentry entries[], int x)
{
	...
	if(!strcmp(gcFunction,"tJobTools"))
	{
		...
		else if(!strcmp(gcCommand,"Confirm Purge"))
		{
			ProcesstJobVars(entries,x);
...
void ExttJobButtons(void)
{
	...
	switch(guMode)
		...
		case 4000:
			printf("<p><u>Confirm job queue purge</u><br>");
			printf("<input class=lwarnButton title='No undo available!' type=submit name=gcCommand value='Confirm Purge'>\n");
		break;
		...
		default:
			...
			if(guPermLevel>=12)
				printf("<input class=lwarnButton title='No undo available!' type=submit name=gcCommand value='Purge'><br>\n");
...
void tJobNavList(void)
{
        ...
        MYSQL_ROW field;
	char *cColor="";
	unsigned localuJobStatus=0;
	
	printf("<p><u>tJobNavList</u><br>\n");
	
	ExtSelectSearch("tJob","tJob.uJob,tJob.cLabel,tJob.cServer,tJob.uJobStatus","tJob.cLabel","",
			"(uJobStatus!=3 AND uJobStatus!=6)",0);
	...
	{	
		...
		{
			sscanf(field[3],"%u",&localuJobStatus);
			if(localuJobStatus==5) 
				cColor="red";
			else
				cColor="black";
