	...
	if(!cResult[0])
	{
		...
		if(mysql_errno(&gMysql))
			...
			if(strstr(mysql_error(&gMysql)," doesn't exist"))
                	{
				CreatetTemplateSet();
				unxsVZ("New tTemplateSet table created");
                	}
			else
			{
...
void NewtTemplateSet(unsigned uMode)
{
	...
	unxsVZLog(uTemplateSet,"tTemplateSet","New");
...
void DeletetTemplateSet(void)
{
	...
	{
		unxsVZLog(uTemplateSet,"tTemplateSet","Del");
		tTemplateSet(LANG_NBR_RECDELETED);
	}
	else
	{
		unxsVZLog(uTemplateSet,"tTemplateSet","DelError");
...
void ModtTemplateSet(void)
{
	...
	unxsVZLog(uTemplateSet,"tTemplateSet","Mod");
	tTemplateSet(gcQuery);
...
void tTemplateSetList(void)
{
	...
	for(guN=0;guN<(guEnd-guStart+1);guN++)
	{
		...
		if(luTime6)
			...
			sprintf(cBuf6,"---");
		printf("<td><a class=darkLink href=unxsVZ.cgi?gcFunction=tTemplateSet&uTemplateSet=%s>%s</a>"
			,field[0]
			,field[0]);
...
{
	sprintf(gcQuery,"CREATE TABLE IF NOT EXISTS tTemplateSet ("
			" uTemplateSet INT UNSIGNED PRIMARY KEY AUTO_INCREMENT,"
