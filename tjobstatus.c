/*
FILE
	tJobStatus source code of unxsVZ.cgi
PURPOSE
	Job queue table job status codes
AUTHOR/LEGAL
	...
	if(!cResult[0])
	{
		...
		if(mysql_errno(&gMysql))
			...
			if(strstr(mysql_error(&gMysql)," doesn't exist"))
                	{
				CreatetJobStatus();
				unxsVZ("New tJobStatus table created");
                	}
			else
			{
...
void NewtJobStatus(unsigned uMode)
{
	...
	unxsVZLog(uJobStatus,"tJobStatus","New");
...
void DeletetJobStatus(void)
{
	...
	{
		unxsVZLog(uJobStatus,"tJobStatus","Del");
		tJobStatus(LANG_NBR_RECDELETED);
	}
	else
	{
		unxsVZLog(uJobStatus,"tJobStatus","DelError");
...
void ModtJobStatus(void)
{
	...
	unxsVZLog(uJobStatus,"tJobStatus","Mod");
	tJobStatus(gcQuery);
...
void tJobStatusList(void)
{
	...
	for(guN=0;guN<(guEnd-guStart+1);guN++)
	{
		...
		if(luTime6)
			...
			sprintf(cBuf6,"---");
		printf("<td><a class=darkLink href=unxsVZ.cgi?gcFunction=tJobStatus&uJobStatus=%s>%s</a>"
			,field[0]
			,field[0]);
...
{
	sprintf(gcQuery,"CREATE TABLE IF NOT EXISTS tJobStatus ("
			" uJobStatus INT UNSIGNED PRIMARY KEY AUTO_INCREMENT,"
