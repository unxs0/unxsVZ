	...
	if(!cResult[0])
	{
		...
		if(mysql_errno(&gMysql))
			...
			if(strstr(mysql_error(&gMysql)," doesn't exist"))
                	{
				CreatetServer();
				unxsSPS("New tServer table created");
                	}
			else
			{
...
void NewtServer(unsigned uMode)
{
	...
	unxsSPSLog(uServer,"tServer","New");
#endif
...
void DeletetServer(void)
{
	...
	{
#ifdef ISM3FIELDS
		unxsSPSLog(uServer,"tServer","Del");
#endif
	...
	{
#ifdef ISM3FIELDS
		unxsSPSLog(uServer,"tServer","DelError");
#endif
...
void ModtServer(void)
{
	...
	unxsSPSLog(uServer,"tServer","Mod");
#endif
	tServer(gcQuery);
