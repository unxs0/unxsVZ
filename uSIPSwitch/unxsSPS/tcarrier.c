...
void ProcesstCarrierListVars(pentry entries[], int x);

 //In tCarrierfunc.h file included below
...
void ProcesstCarrierVars(pentry entries[], int x)
{
	...
	ExtProcesstCarrierVars(entries,x);
...
void ProcesstCarrierListVars(pentry entries[], int x)
{
        ...
        for(i=0;i<x;i++)
        {
                ...
                {
                        sscanf(entries[i].name+2,"%u",&uCarrier);
                        guMode=2002;
                        tCarrier("");
                }
        }
...
int tCarrierCommands(pentry entries[], int x)
{
	...
	else if(!strcmp(gcFunction,"tCarrierList"))
	{
		ProcessControlVars(entries,x);
		ProcesstCarrierListVars(entries,x);
		tCarrierList();
	}
...
void tCarrier(const char *cResult)
{
	MYSQL_RES *res;
	MYSQL_RES *res2;
	MYSQL_ROW field;
	...
	if(!cResult[0])
	{
		...
		if(mysql_errno(&gMysql))
			...
			if(strstr(mysql_error(&gMysql)," doesn't exist"))
                	{
				CreatetCarrier();
				unxsSPS("New tCarrier table created");
                	}
			else
			{
	...
	printf("<input type=hidden name=gcFunction value=tCarrierTools>");
	...
	//
	OpenFieldSet("tCarrier Record Data",100);
