	...
	if(!cResult[0])
	{
		...
		if(mysql_errno(&gMysql))
			...
			if(strstr(mysql_error(&gMysql)," doesn't exist"))
                	{
				CreatetGlossary();
				unxsSPS("New tGlossary table created");
                	}
			else
			{
...
void NewtGlossary(unsigned uMode)
{
	...
	unxsSPSLog(uGlossary,"tGlossary","New");
#endif
...
void DeletetGlossary(void)
{
	...
	{
#ifdef ISM3FIELDS
		unxsSPSLog(uGlossary,"tGlossary","Del");
#endif
	...
	{
#ifdef ISM3FIELDS
		unxsSPSLog(uGlossary,"tGlossary","DelError");
#endif
...
void ModtGlossary(void)
{
	...
	unxsSPSLog(uGlossary,"tGlossary","Mod");
#endif
	tGlossary(gcQuery);
