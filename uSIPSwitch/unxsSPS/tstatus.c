	...
	if(!cResult[0])
	{
		...
		if(mysql_errno(&gMysql))
			...
			if(strstr(mysql_error(&gMysql)," doesn't exist"))
                	{
				CreatetStatus();
				unxsSPS("New tStatus table created");
                	}
			else
			{
...
void NewtStatus(unsigned uMode)
{
	...
	unxsSPSLog(uStatus,"tStatus","New");
#endif
...
void DeletetStatus(void)
{
	...
	{
#ifdef ISM3FIELDS
		unxsSPSLog(uStatus,"tStatus","Del");
#endif
	...
	{
#ifdef ISM3FIELDS
		unxsSPSLog(uStatus,"tStatus","DelError");
#endif
...
void ModtStatus(void)
{
	...
	unxsSPSLog(uStatus,"tStatus","Mod");
#endif
	tStatus(gcQuery);
