	...
	if(!cResult[0])
	{
		...
		if(mysql_errno(&gMysql))
			...
			if(strstr(mysql_error(&gMysql)," doesn't exist"))
				...
				unxsSPS("New tConfiguration table created");
                	}
			else
			{
...
void NewtConfiguration(unsigned uMode)
{
	...
	unxsSPSLog(uConfiguration,"tConfiguration","New");
...
void DeletetConfiguration(void)
{
	...
	{
		unxsSPSLog(uConfiguration,"tConfiguration","Del");
		tConfiguration(LANG_NBR_RECDELETED);
	}
	else
	{
		unxsSPSLog(uConfiguration,"tConfiguration","DelError");
